...
$(document).ready(function () {
    ...
    $("#fancybox-manual-c").click(function () {
        $.fancybox.open([{href: "1_b.jpg", title: "My title"}, {
            href: "2_b.jpg",
            title: "2nd title"
...
// };
window.onload = function () {
    baron({root: "body"}).dispose();
};
$(function () {
    var liveSearch = $("#right"),
        searchCache = [],
        searchOk = $("#search_ok"),
        search;
    $(".search-concs").keyup(throttle(function () {
        ...
        liveSearch.attr({"data-code": code});
        var searchResults = liveSearch.find(".conc_container");
        if (search.length > 2) {
            if (searchCache[search]) {
                searchResults.html(searchCache[search]);
            } else {
                $.ajax({
                    type: "POST",
    ...
    $(".productname").mouseup(getSelectionToSearch);
    
    function getSelectionToSearch() {
    ...
    $(".search-product-code").one("click", function () {
        searchOk.addClass("search_loader");
        search = $(this).text();
        var live_search = $("#right");
