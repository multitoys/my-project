<?php
    ...
    for ($i=0;$i<sizeof($lines); $i++)
        ...
        = explode ("\|", substr($lines[$i],$lineoffset));
        list($linedate,$linetime) = explode(" ",$fulldate);
	...
	if( preg_match("/\sWA\:\s/",$line) == "0" )
        ...
        list($dbkey, $u_id, $fulldate, $app_id, $action_type, $data, $client_type, $client_ip)
            = explode ("\|", substr($line,$lineoffset));
        if ( $dbkey == "" or preg_match("/web[1-3]/",$dbkey) == "1" or $fulldate == "" or $app_id == "" or $action_type == "" or $client_type == "" or $client_type == "")
    	    {
    		$errorlines = dirname(__FILE__) . "/../../temp/log/errorlines.log";
    		$err_handle = fopen($errorlines,'a');
    		fwrite($err_handle,"$line\n");
