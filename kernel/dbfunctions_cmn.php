<?php
    ...
    function updateIdentityScreenAccess($ID, $ID_Type, $screenAccessData, $auxRightsData, &$kernelStrings)
        //
        // Updates user or group access rigths to forms
        //
        //		Parameters:
        //			$ID - identity identifier
        //			$ID_Type - identity type
        //			$screenAccessData - an array containing access information. Format:
        //				array( APP_ID1=>aray(SCR_ID1=>1, SCR_ID2=>1) ... )
        //			$auxRightsData - aux access data
        ...
        //
    {
        ...
        global $qr_addUserScreenAccess;
        global $qr_deleteGroupScreenAccess;
        global $qr_addGroupScreenAccess;
        global $qr_deleteAuxGroupAccess;
        global $qr_deleteAuxUserAccess;
        global $qr_addAuxGroupAccess;
        global $qr_addAuxUserAccess;
        ...
        if ($ID_Type == IDT_USER) {
            $params = array("U_ID" => $ID);
            if (PEAR::isError($res = exec_sql($qr_deleteUserScreenAccess, $params, $errorStr, false)))
                return PEAR::raiseError($kernelStrings[ERR_QUERYEXECUTING]);

            if (PEAR::isError($res = exec_sql($qr_deleteAuxUserAccess, $params, $errorStr, false)))
                return PEAR::raiseError($kernelStrings[ERR_QUERYEXECUTING]);
        } else {
            $params = array("UG_ID" => $ID);
            if (PEAR::isError($res = exec_sql($qr_deleteGroupScreenAccess, $params, $errorStr, false)))
                return PEAR::raiseError($kernelStrings[ERR_QUERYEXECUTING]);

            if (PEAR::isError($res = exec_sql($qr_deleteAuxGroupAccess, $params, $errorStr, false)))
                return PEAR::raiseError($kernelStrings[ERR_QUERYEXECUTING]);
        }

        if (is_array($screenAccessData))
            foreach ($screenAccessData as $app_id => $screen_data)
                foreach ($screen_data as $scr_id => $val) {
                    if ($ID_Type == IDT_USER) {
                        $params = array("U_ID" => $ID, APP_ID => $app_id, "SCR_ID" => $scr_id);

                        if (PEAR::isError($res = exec_sql($qr_addUserScreenAccess, $params, $errorStr, false)))
                            return PEAR::raiseError($kernelStrings[ERR_QUERYEXECUTING]);
                    } else {
                        $params = array("UG_ID" => $ID, APP_ID => $app_id, "SCR_ID" => $scr_id);

                        if (PEAR::isError($res = exec_sql($qr_addGroupScreenAccess, $params, $errorStr, false)))
                            return PEAR::raiseError($kernelStrings[ERR_QUERYEXECUTING]);
                    }
                }

        if (is_array($auxRightsData)) {
            foreach ($auxRightsData as $app_id => $rightData) {
                foreach ($rightData as $right_id => $val) {
                    if ($ID_Type == IDT_USER) {
                        $params = array("U_ID" => $ID, APP_ID => $app_id, "APP_ID" => $app_id, 'AGA_ID' => $right_id);

                        if (PEAR::isError($res = exec_sql($qr_addAuxUserAccess, $params, $errorStr, false)))
                            return PEAR::raiseError($kernelStrings[ERR_QUERYEXECUTING]);
                    } else {
                        $params = array("UG_ID" => $ID, APP_ID => $app_id, "APP_ID" => $app_id, 'AGA_ID' => $right_id);

                        if (PEAR::isError($res = exec_sql($qr_addAuxGroupAccess, $params, $errorStr, false)))
                            return PEAR::raiseError($kernelStrings[ERR_QUERYEXECUTING]);
                    }
                }
            }
        ...
        //
    {
        ...
        $result[MAILFORMAT] = $personalSettings[MAILFORMAT];
        ...
        //
    {
        ...
        $res = writeUserCommonSetting($userdata["U_ID"], MAILFORMAT, $userdata[MAILFORMAT], $kernelStrings);
        if (PEAR::isError($res))
            return PEAR::raiseError($kernelStrings[ERR_SAVINGUSERSETTINGS]);
...
//			$dbh = mysql_connect( $db_host, DB_ADMIN_USER, DB_ADMIN_PASSWORD );
            $dbh = mysql_connect('localhost', 'multitoy_multi', 'V2AU3Fsf');
            if (!$dbh) {
                PEAR::raiseError(mysql_error());
        ...
        //
    {
        ...
        $tablesList = array('APPLICATION', 'COMPANY',
            'CURRENCY', 'SCREEN',
            'USERSCREENACCESS', 'SCR_GROUP_ACCESS',
            'WBS_USER', 'CONTACT', 'UNSUBSCRIBER',
            'CFOLDER', 'CTYPE',
            'CFACCESS', 'CFGROUPACCESS',
            'UGROUP', 'UGROUP_USER',
            'FILE_IMPORT_FORMAT', 'APPSETTINGS',
            'DISK_USAGE', 'USER_DISK_QUOTA',
            'SMS_BALANCE', 'SMS_CREDIT_HISTORY',
            'SMS_HISTORY', 'AUX_GROUP_ACCESS',
            'AUX_USER_ACCESS', 'CLIST',
            'CLIST_CONTACT', 'CLIST_GROUP',
            'CLIST_FOLDER', 'U_ACCESSRIGHTS',
...
*/
        ...
        //
    {
        ...
        if (!isset($databaseInfo[HOST_DBSETTINGS][HOST_TRIALDATASOURCE])) {
            // Create Contact Folder "_Unsorted"
            //
            if (PEAR::isError($res = db_query($qr_host_addFolder, array()))) {
                rollbackDbCreation(1, $databaseName, $existingDbName, $mySQLUser, $mySQLPassword, $existTables);
            ...
            // Create Contact Type
            //
            $con_settingsPath = sprintf("%skernel/includes/con_settings.xml", WBS_DIR);
            $con_settings = @file($con_settingsPath);
            $con_settings = implode(" ", $con_settings);

            if (PEAR::isError($res = db_query($qr_host_addContactType, array('CT_SETTINGS' => $con_settings)))) {
                rollbackDbCreation(1, $databaseName, $existingDbName, $mySQLUser, $mySQLPassword, $existTables);
        ...
        $contactData['CF_ID'] = DEF_CONTACT_FOLDER;
        ...
        $contactData['C_CREATEUSERNAME'] = getArrUserName($contactData);
        ...
        $sql = "UPDATE USER_SETTINGS SET U_ID = '!U_ID!'";
        ...
        //
    {
        if (!$useCookies) {
            ...
            if (PEAR::isError($res)) {
                if (onWebAsystServer()) {
                    $wbs = new WbsUpdater("SYSTEM");
                    $current_version = $wbs->getCurrentVersion();
                    if ($current_version[0] == 278) {
                        $wbs->saveCurrentVersion(277);
                    }
                }
        ...
        //
    {
        ...
        $mailFormat = null;
        if (strlen($recipient_U_ID))
            $mailFormat = readUserCommonSetting($recipient_U_ID, MAILFORMAT);

        if (!$mailFormat)
            $mailFormat = MAILFORMAT_TEXT;
        ...
        return @mail($recipientMail, $subject, $body, $headers, $returnHeader);
    }
