...
// {{{ class Mail_IMAP
    class Mail_IMAP
    {
         ...
         *                            (optional), one of E_ALL or 0, tells Mail_IMAP to report more about the messages
         *                            it is parsing and where hacks are being used, such as fallback PIDs. This level
         *                            of error reporting can become annoying, to turn it off, set to 0.
         *
         * @access    public
         ...
         *                                                As of Mail_IMAP 1.1.0 the $options argument accepts an action
         *                                                for retrieving various mailbox information. If set to
         *                                                MAIL_IMAP_GET_INFO (the default action) Mail_IMAP::connect
         *                                                will make a call to {@link getMailboxInfo}. If set to
         *                                                MAIL_IMAP_NO_INFO this call will not be made. In the upcoming
         *                                                Mail_IMAP 2.0.0 release the $options argument will be no
         *                                                longer specify optional flags for {@link imap_open} and will
         *                                                server exclusively as an action toggle for {@link
         *                                                getMailboxInfo}.
         *
         ...
         *
         * @param    int $options (optional) sets the second argument of imap_close (DEPRECATED, use
         *                        $this->option['close'] instead)
         *
         * @return   PEAR_Error|TRUE
         * @access   public
         * @see      imap_close
                ...
                if ($this->_ftype[$mid][$i] == 'message/rfc822' || $this->_ftype[$mid][$i] == 'multipart/related' || $this->_ftype[$mid][$i] == 'multipart/alternative') {
                    ...
                    switch (true) {
                        case (!empty($new_pid)):
                            $pid = $new_pid;
                            break;
                        case (empty($new_pid) && $getPart == 'text/html'):
                            return ($attempt == 1) ? Mail_IMAP::getBody($mid, $pid, $action, 'text/plain', $options, 2) : false;
                        case (empty($new_pid) && $getPart == 'text/plain'):
                            return ($attempt == 1) ? Mail_IMAP::getBody($mid, $pid, $action, 'text/html', $options, 2) : false;
                    }
                }
        ...
        function getDefaultPid(&$mid, $getPart = 'text/html', $attempt = 1)
        {
            ...
            switch ($getPart) {
                case 'text/html':
                    $ret = ($attempt == 1) ? Mail_IMAP::getDefaultPid($mid, 'text/plain', 2) : false;
                case 'text/plain':
                    $ret = ($attempt == 1) ? Mail_IMAP::getDefaultPid($mid, 'text/html', 2) : false;
                default:
                    $ret = false;
            }
         ...
         * @param    int    $options        (optional) flags/options for imap_fetchbody (DEPRECATED, use
         *                                  $this->option['fetchbody'])
         *
         * @return   Array|BOOL
         * @access   public
         * @see      getParts
         ...
         * @param    int   $options
         *                               (optional) sets the forth argument of {@link imap_setflag_full} or
         *                               {@imap_clearflag_full}.
         *
