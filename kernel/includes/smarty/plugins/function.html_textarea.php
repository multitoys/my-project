	...
	function smarty_function_html_textarea($params, &$smarty){
		...
		foreach ($languages as $languageEntry){
			...
			<tr>
				...
				':'').'
				<td>
					<textarea lang="'.$lang_iso2.'" name="'.xHtmlSpecialChars($input_name).'" '.(isset($id)?" id='{$id}_{$lang_iso2}'":'').(isset($rows)?" rows='{$rows}'":'').(isset($cols)?" cols='{$cols}'":'').(isset($style)?" style='{$style}'":'').(isset($class)?" class='{$class}'":'').'>'.xHtmlSpecialChars($values[$dbfield.'_'.$lang_iso2]).'</textarea>
				</td>
			</tr>';
