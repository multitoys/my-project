...
$qr_ur_selectUserItems = "( SELECT W.U_ID, C.C_FIRSTNAME, C.C_LASTNAME, C.C_MIDDLENAME, C.C_NICKNAME, C.C_EMAILADDRESS, NULL AS AR_PATH, NULL AS AR_OBJECT_ID, NULL AS AR_VALUE, NULL AS AR_AUX FROM WBS_USER AS W LEFT JOIN CONTACT AS C USING( C_ID ) ) UNION  ( SELECT W.U_ID, C.C_FIRSTNAME, C.C_LASTNAME, C.C_MIDDLENAME, C.C_NICKNAME, C.C_EMAILADDRESS, AR.AR_PATH, AR.AR_OBJECT_ID, AR.AR_VALUE, AR.AR_AUX   FROM WBS_USER AS W LEFT JOIN CONTACT AS C USING( C_ID ) LEFT JOIN U_ACCESSRIGHTS AS AR ON W.U_ID = AR.AR_ID  WHERE AR.AR_PATH='!AR_PATH!' AND AR.AR_OBJECT_ID='!AR_OBJECT_ID!' ) ORDER BY UPPER(CONCAT(IF(C_FIRSTNAME is not null, C_FIRSTNAME, ''), IF(C_MIDDLENAME is not null, C_MIDDLENAME, ''), IF(C_LASTNAME is not null, C_LASTNAME, ''), IF(C_NICKNAME is not null, C_NICKNAME, ''), IF(C_EMAILADDRESS is not null, C_EMAILADDRESS, '')))";
...
class UR_RightsManager extends UR_RightsObject
{
    ...
    function __loadItemsCache( $object_id=null )
    {
        ...
        {
            $row[ UR_ID ] = ( $action == UR_ACTION_EDITGROUP ) ? $row["UG_ID"] : $row["U_ID"];

            $row[UR_ITEMNAME] = ($action == UR_ACTION_EDITGROUP) ? $row[ "UG_NAME" ] : getArrUserName( $row );
    ...
    function __SaveRightValue( $path, $id, $type, $value=0, $aux=null, $updateCache = true )
    {
        ...
        if ( is_array( $aux ) )
            $saveArr[UR_AUX] = base64_encode( serialize( $aux ) );
        else
            $saveArr[UR_AUX] = null;
...
class UR_RO_Trigger extends UR_RightsObject
{
    ...
    function RenderItem( &$data )
    {
        ...
        {
            if ( $this->__root->__renderOptions[UR_ACTION] == UR_ACTION_EDITUSER )
                $render .= "<tr><td>".$row[UR_ITEMNAME]."</td><td>$id</td>";
            else
...
class UR_RO_FoldersTree extends UR_RightsObject
{
    ...
    function RenderItem( &$data )
    {
        ...
        {
            if ( $this->__root->__renderOptions[UR_ACTION] == UR_ACTION_EDITUSER )
                $render .= "<tr><td>".$valueRow[UR_ITEMNAME]."</td><td>$id</td>";
            else
