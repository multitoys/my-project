<?php
    ...
    $setings_host = SystemSettings::get('DB_HOST');
    $setings_user = SystemSettings::get('DB_USER');
    $setings_pass = SystemSettings::get('DB_PASS');
    $setings_db = SystemSettings::get('DB_NAME');
...
TAG
    ...
    if (($handle = fopen($filename, 'r')) !== false) {
        ...
        $mysqli = new mysqli($setings_host, $setings_user, $setings_pass, $setings_db);
        ...
        $id_min = 77777;
        $id_max = 100003;
        $result = new Result($mysqli, $id_min, $id_max);
        print $result->getResult();

        if (!($stmt = $mysqli->prepare('SELECT categoryID FROM SC_categories WHERE name_ru = ?'))) {
            echo 'Не удалось подготовить запрос: ('.$mysqli->errno.') '.$mysqli->error;
        }

        if (!$stmt->bind_param('s', $category_extra)) {
            echo 'Не удалось привязать параметры: ('.$stmt->errno.') '.$stmt->error;
        }

        $stmt->bind_result($category_const);

        // Идентификатор категории "новинки"
        $category_extra = 'Новинки';
        $stmt->execute();
        $stmt->fetch();
        define('CAT_NOVINKI_ID', $category_const);
        echo CAT_NOVINKI_ID.'<br>';

        // Идентификатор категории "акция"
        $category_extra = 'Акция';
        $stmt->execute();
        $stmt->fetch();
        define('CAT_AKCIA_ID', $category_const);
        echo $category_const.'<br>';

        // Идентификатор категории "акция "Баллы"
        $category_extra = 'Акция - Супертрехдневка';
        $stmt->execute();
        $stmt->fetch();
        define('CAT_AKCIA_BALLY_ID', $category_const);
        echo $category_const.'<br>';

        // Идентификатор категории "Все бонусные товары"
        $category_extra = 'Все бонусные товары';
        $stmt->execute();
        $stmt->fetch();
        define('CAT_BONUS_ID', $category_const);
        echo $category_const.'<br>';

        // Идентификатор категории "Все товары под заказ"
        $category_extra = 'Все товары под заказ';
        $stmt->execute();
        $stmt->fetch();
        define('CAT_ZAKAZ_ID', $category_const);
        echo $category_const.'<br>';
        $stmt->free_result();
        $stmt->close();
...
//            }
//
//            if (!$tag = GetValue('id', 'SC_tags', "name = '$brand'")) {
//
//            }
//        }
//        fclose($handle);
