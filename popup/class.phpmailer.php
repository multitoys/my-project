...
class PHPMailer {
    ...
    while (!$foundSplitPos) {
      ...
      if ($encodedCharPos !== false) {
        ...
        if ($dec < 128) { // Single byte character.
          ...
          // Reduce maxLength to explode at start of character
  ...
  public function EncodeHeader ($str, $position = 'text') {
    ...
    if (strlen($str)/3 < $x) {
      $encoding = 'B';
      if (function_exists('mb_strlen') && ($this->HasMultiBytes($str) || true)) {
    ...
    $encoded = preg_replace('/^(.*)$/um', " =?".$this->CharSet."?$encoding?\\1?=", $encoded);
