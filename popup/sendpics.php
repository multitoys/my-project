<?php
 ...
 // $prefix     = GetValue("settings_value","SC_settings","settings_constant_name = 'CONF_ORDERID_PREFIX'");
  $email_to   = GetValue("settings_value","SC_settings","settings_constant_name = 'CONF_GENERAL_EMAIL'");

  $query = "SELECT  code_1c ,  enabled 
FROM  SC_products
WHERE  default_picture IS NULL 
ORDER BY  SC_products.code_1c ASC"; // Новые заказы
  $res = mysql_query($query) or die(mysql_error()."<br>$query");
  echo("empty_pics:".mysql_num_rows($res)."<br><br>");
  
  $pics="IS NULL"
  $file_content = MakeXLS($pics);
    //$file_content1 = 123, 1);
  /* while($row = mysql_fetch_object($res))
  {
    
    $name  = GetValue("CONCAT(first_name, ' ', last_name)", 'SC_customers', "customerID = $row->customerID"); */
    $filename = "nonfotos.xls";
    $filename0 = my_translit($filename);
    $thm = "Ннет фото";
    $msg1 = 'нет фото';//GetNonAutomatic($row->orderID);
    //$msg = 'Пазлы и товары под заказ с MULTITOYS';
    // Отправляем почтовое сообщение
    $o = send_mail2($email_to, $thm, $msg1, $filename0, $file_content);
   // $o = send_mail2($email_to, $thm, $msg, $filename0, $file_content1);
  /*   if ($o == 1)
    {
      $query = "UPDATE SC_orders SET statusID = 5 WHERE orderID = $row->orderID";
      $res2 = mysql_query($query) or die(mysql_error()."<br>$query");
    } */
    $o = $o?"$filename0 -> YES!<br>":"$filename0 -> (((<br>";
    echo($o);
...
$mod = 'NOT SC_categories.allow_products_comparison OR SC_products.zakaz!=0';
     ...
     $query = "SELECT  code_1c ,  product_code,enabled, name 
FROM  SC_products 
WHERE  default_picture IS NULL 
ORDER BY  SC_products.code_1c ASC";     
    $res = mysql_query($query) or die(mysql_error()."<br>$query");
   /*  if (mysql_num_rows($res))
    { */
  ...
  /*     $textout .= xlsWriteLabel(0,9,"Адрес");
      ...
      $textout .= xlsWriteLabel(0,0,"Код");
      $textout .= xlsWriteLabel(0,1,"Артикул");
      $textout .= xlsWriteLabel(0,2,"Наименование");
      $textout .= xlsWriteLabel(0,3,"В наличии");
/*       $textout .= xlsWriteLabel(0,4,"Товар");
      $textout .= xlsWriteLabel(0,5,"Количество");
      $textout .= xlsWriteLabel(0,6,"Цена");
      ...
      {
        $no++;
        $textout .= xlsWriteNumber($no,0,$row->code_1c);
        $textout .= xlsWriteLabel ($no,1,$row->product_code);
        $textout .= xlsWriteLabel ($no,2,$row->name);
        $textout .= xlsWriteLabel ($no,3,$row->enabled);
