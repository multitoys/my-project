var hs = {
    ...
    focusTopmost: function () {
        ...
        if (topmostKey == -1)hs.focusKey = -1; else hs.expanders[topmostKey].focus();
    },
    closeId: function (id) {
        return hs.close(id);
    },
    ...
    getParam: function (a, param) {
        try {
            ...
            eval('var arr = {' + s + '};');
            if (arr[param])return arr[param]; else return hs[param];
        } catch (e) {
            return hs[param];
        }
    },
    ...
    previousOrNext: function (el, op) {
        ...
        try {
            hs.getAdjacentAnchor(exp.key, op).onclick();
        } catch (e) {
        }
        try {
            exp.doClose();
    ...
    mouseClickHandler: function (e) {
        ...
        if (e.target.form)return;
        var fobj = e.target;
        while (fobj.parentNode && !(fobj.className && fobj.className.match(/highslide-(image|move|html)/))) {
...
HsOutline = function (outlineType, onLoad) {
    ...
    hs.genContainer();
    this.table = hs.createElement('table', {cellSpacing: 0}, {
        visibility: 'hidden',
        position: 'absolute',
        zIndex: hs.zIndexCounter++,
...
HsExpander = function (a, params, custom, contentType) {
    ...
    for (i = 0; i < hs.overrides.length; i++) {
        ...
        if (params && typeof params[name] != 'undefined')this[name] = params[name]; else this[name] = hs[name];
    }
    var el;
    if (params && params.thumbnailId)el = hs.$(params.thumbnailId); else el = a.getElementsByTagName('IMG')[0];
    if (!el)el = a;
    for (i = 0; i < hs.expanders.length; i++) {
    ...
    }, {visibility: 'hidden', position: 'absolute', zIndex: hs.zIndexCounter++}, null, true);
    this.wrapper.onmouseover = function (e) {
        try {
            hs.expanders[key].onMouseOver(e);
        } catch (e) {
        }
    };
    this.wrapper.onmouseout = function (e) {
        try {
            hs.expanders[key].onMouseOut(e);
        } catch (e) {
        }
    };
...
HsExpander.prototype.displayLoading = function () {
    ...
    if (!hs.loading) {
        ...
        }, {position: 'absolute', opacity: hs.loadingOpacity}, hs.container);
    }
...
HsExpander.prototype.justify = function (p) {
    ...
    if (p.target && hs.$(p.target)) {
        ...
        if (p.justify == 'center')p.min = Math.round(p.scroll + (p.clientSpan - p.span - p.marginMax) / 2); else p.min = Math.round(p.min - ((p.span - p.thumbSpan) / 2));
        ...
        if (p.min + p.span > p.scroll + p.clientSpan - p.marginMax) {
            if (hasMovedMin && allowReduce)p.span = p.clientSpan - p.marginMin - p.marginMax; else if (p.span < p.clientSpan - p.marginMin - p.marginMax) {
...
HsExpander.prototype.changeSize = function (dir, x1, y1, w1, h1, x2, y2, w2, h2, dur, steps, oo1, oo2) {
    var dW = (w2 - w1) / steps, dH = (h2 - h1) / steps, dX = (x2 - x1) / steps, dY = (y2 - y1) / steps, dOo = (oo2 - oo1) / steps, t, exp = "hs.expanders[" + this.key + "]";
    for (i = 1; i <= steps; i++) {
        w1 += dW;
        h1 += dH;
        x1 += dX;
        ...
        var s = "try {";
        if (i == 1) {
            s += exp + ".content.style.visibility = 'visible';" + "if (" + exp + ".thumb.tagName == 'IMG' && hs.hideThumbOnExpand) " + exp + ".thumb.style.visibility = 'hidden';"
        }
        if (i == steps) {
            w1 = w2;
            h1 = h2;
        ...
        s += exp + "." + this.contentType + "SetSize(" + Math.round(w1) + ", " + Math.round(h1) + ", " + Math.round(x1) + ", " + Math.round(y1) + ", " + Math.round(oo1);
        s += ");} catch (e) {}";
        setTimeout(s, t);
    }
    if (dir == 1) {
...
HsExpander.prototype.doClose = function () {
    ...
    try {
        ...
        if (this.objOutline) {
            if (this.outlineWhileAnimating)this.positionOutline(x, y, w, h); else if (this.preserveContent)this.objOutline.table.style.visibility = 'hidden'; else this.objOutline.destroy();
        }
...
HsExpander.prototype.onEndClose = function () {
    ...
    if (this.isHtml && this.preserveContent)this.sleep(); else {
        if (this.objOutline && this.outlineWhileAnimating)this.objOutline.destroy();
        hs.purge(this.wrapper);
        if (hs.ie && hs.ieVersion() < 5.5)this.wrapper.innerHTML = ''; else this.wrapper.parentNode.removeChild(this.wrapper);
    }
...
HsExpander.prototype.createOverlay = function (el, position, hideOnMouseOut, opacity) {
    ...
    var overlay = hs.createElement('div', null, {
        'left': 0,
        'top': 0,
        'position': 'absolute',
        'zIndex': 3,
