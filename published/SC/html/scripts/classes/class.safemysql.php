<?php
     ...
     * Первый служит для добавления строк, а так же всех типов данных, приравненных к ним.
     * Второй - целых чисел.
     * Третий - идентификаторый - служит для динамической вставки в запрос имен полей и таблиц.
     * Он требуется потому, что правила форматирования идентификаторов отличаются от правил форматирования строк.
     * Ограничивается он не одинарной кавычкой, а обратным апострофом, а экранируется не слешем, а удвоением.
     * Назначение четвёртого - подставлять в запрос уже обработанные части запроса.

     * Последние два введены для удобства. Очень часто бывает нужно подставить массив в оператор IN().
     * Классическими средствами это сделать затруднительно, а с помощью плейсхолдера - не сложнее, чем строку.
     ...
     * В случае, если они добавляются в запрос динамически, то их надо помещать в запрос только из белого списка,
     * заранее прописанного в скрипте.
     * Следует учесть, что идентификаторы тоже желательно добавлять из белого списка, во всяком случае,
     * при поступлении данных от пользователя, поскольку ему может быть запрещён доступ к определенным полям таблицы.
     * Для такой фильтрации служат методы filterArray() и whiteList()
     * filterArray() выбрасывает из переданного первым параметром массива все элементы, которых нет во втором массиве.
     * Используется для фильтрации массивов, передаваемых для плейсхолдера ?u
     ...
     * getIndCol($key,$query,$par1,$par2, ...)  - массив скаляров, индексированный полем из первого параметра.
     *                                           Незаменимо для составления словарей вида key => value
     */
