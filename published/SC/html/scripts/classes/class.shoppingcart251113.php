		...
		function saveToOrderedCarts($orderID, $shipping_info, $billing_info, $calculate_tax = true){
		  ...
		  $skidka        = mysql_fetch_object(mysql_query($query))->skidka;
			...
			for ($i=0; $i<$tc; $i++){
			  ...
			  $maxskidka = $product['skidka'];
			  $oneskidka = min($maxskidka, $skidka);
			  $price = round($price*(1-$oneskidka/100),2);

			  $productComplexName = ($oneskidka>0)?"$productComplexName (-$oneskidka%)":$productComplexName;
			...
			foreach ($r_aItem as $aItem){
			  ...
			  $oneskidka = min($maxskidka, $skidka);
				...
				$cart_content[] = array(
					...
					'costUC' =>	round($aPrice->getData()*(1-$oneskidka/100),2),
					'cost' => show_price($aItem->getChildData('quantity')*PaymentModule::_convertCurrency(round($aPrice->getData()*(1-$oneskidka/100),2), $aPrice->attribute('currency'), 0), 0),
