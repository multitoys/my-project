...

{
	...
	// echo($customer->is_special_price.'<br>');
	// echo($customer->ignore_skidka.'<br>');
	// echo($r["SpecialPrice"].'<br>');
	// echo($r["Price"].'<br>');
	// echo($customer->skidka.'<br>');
	// echo($r["skidka"].'<br>');

	$full_price = priceDiscount($customer->is_special_price, $r["SpecialPrice"], , $r["Price"], $customer->skidka, $r["skidka"]);
	  ...
	  switch ($sort) {
		...
		while ($cart_item = db_fetch_assoc($q)){
		  ...
		  $oneskidka = min($maxskidka, $skidka);
			...
			"doza"  => $cart_item["doza"],
			"Bonus" => $Bonus,
			"cost" => $cart_item["Quantity"]*$costUC,
