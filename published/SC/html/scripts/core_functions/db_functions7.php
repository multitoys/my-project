...
//	return $r;
        ...
        $DBHandler->connect(SystemSettings::get('DB_HOST'), SystemSettings::get('DB_USER'), SystemSettings::get('DB_PASS'));
...
//	return mysql_select_db($name);
    ...
    function db_query($s, $ignore_error = false) //database query
    {
        ...
        */
        ...
        $res["resource"] = $DBHandler->query($s);
        /*
        @features My
        */
        if ((isset($_GET['debug']) && $_GET['debug'] == 'time')) {
            ...
            ob_end_clean();
            ob_start();
            print '<br /><span onclick="document.getElementById(\'sqldebug_' . $cnt . '\').style.display=document.getElementById(\'sqldebug_' . $cnt . '\').style.display!=\'block\'?\'block\':\'none\'">+</span>' . sprintf('%03d', $cnt) . ' - <strong>' . $T->timerStop() . '</strong>[m:' . (function_exists('memory_get_usage') ? memory_get_usage() : ' unknown') . '] - ' . xHtmlSpecialChars($s);
            if (in_array($cnt, array(39, 42)) && 0) {
                ?>
                <div id="sqldebug_<?php print $cnt; ?>" style="display:none;">
                    <?php
                        echo $_t;
                    ?>
                </div>
            ...
            $fp = fopen(WBS_DIR . '/temp/queries.html', 'a');
...
//        return mysql_insert_id();
        $DBHandler = &Core::getdbHandler();
        return $DBHandler->getInsertedID();
    }
...
//        return mysql_error();
        $DBHandler = &Core::getdbHandler();
        return mysqli_error($DBHandler->link);
    }
