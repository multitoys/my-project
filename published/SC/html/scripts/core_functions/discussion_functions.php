...
function discAddDiscussion( $productID, $Author, $Topic, $Body){
  ...
  // начало: Письмо админу про новый комментарий
  $mail_from  = GetValue_disc("settings_value","SC_settings","settings_constant_name = 'CONF_GENERAL_EMAIL'");;
  $email_to   = GetValue_disc("settings_value","SC_settings","settings_constant_name = 'CONF_ORDERS_EMAIL'");
  $shop_url   = GetValue_disc("settings_value","SC_settings","settings_constant_name = 'CONF_SHOP_URL'");
  $name_ru    = GetValue_disc("name_ru","SC_products","productID = $productID");
  $body = "
Новый отзыв о товаре `$name_ru`:<br><br>
Автор: $Author ($Topic)<br>
<hr><br>
$Body<br><br>
<a href='$shop_url/published/SC/html/scripts/index.php?did=20&answer=$disc_id'>Ответить на отзыв</a>
&nbsp;&nbsp;
<a href='$shop_url/index.php?productID=$productID'>Перейти к товару</a>
";
  ss_mail($email_to, 'Новый отзыв о товаре', $body, true);
  // конец: Письмо админу про новый комментарий

}

  function GetValue_disc($what, $table, $condition)
  {
