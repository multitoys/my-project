<?php
    ...
    function ordGetOrdersNumAmount($status = null)
    {
        ...
        if (is_null($statusCounts)) {
            ...
            $res = db_phquery_fetch(DBRFETCH_ROW_ALL, 'SELECT `statusID`, COUNT( * ) AS \'count\', SUM(`order_amount`*`currency_value`) AS \'amount\' FROM ?#ORDERS_TABLE GROUP BY `statusID`');
            foreach ($res as $row) {
    ...
    function ordGetOrders($callBackParam, &$count_row, $navigatorParams = null)
    {
        ...
        if (isset($callBackParam["orderStatuses"])) {
            ...
            if (isset($callBackParam["Customer"])) {
                if ($where_clause != "")
                    $where_clause .= " and customer_lastname like '%".$callBackParam["Customer"]."%' or customer_firstname like '%" .$callBackParam["Customer"]. "%'";
                else
                    $where_clause .= " customer_lastname like '%" . $callBackParam["Customer"]. "%' or customer_firstname like '%" .$callBackParam["Customer"]. "%'";
            }
                                              ...
                                              $payment_email_comments_text, $shipping_email_comments_text, $tax)
    {
        ...
        $smarty_mail->assign('order_status_url', str_replace('{URL}', 'http://' . CONF_SHOP_URL . '/index.php?ukey=order_status&orderID=' . $orderID . '&code=' . base64_encode($order['customer_email']), translate('lbl_order_status_history_url')));
    ...
    function ordGetOrder($orderID)
    {
        ...
        if ($order) {
            ...
            $order["order_discount_percent"] = round(100 * $order["order_discount_value"] / $clear_total_price, 1);
...
//							$currentDate );
    ...
    function ordCalculateOrderTax(&$order, $order_content)
    {
        ...
        foreach ($order_content as $_item) {
            $order['tax'] += ($order['currency_value'] * $_item['Price'] * $_item['Quantity']) / 100 */*(100-$order['order_discount'])/100**/
                $_item['tax'];
        }
