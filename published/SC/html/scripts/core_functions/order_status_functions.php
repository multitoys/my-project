...
// *****************************************************************************
// Purpose	Insert prdefined order status into ORDER_STATUSES_TABLE.
...
function ostInstall(){
	...
	$name_inj = LanguagesManager::sql_prepareFieldInsert('status_name', translate("ordr_status_cancelled"));
	db_phquery("INSERT ?#ORDER_STATUSES_TABLE ( {$name_inj['fields']}, sort_order ) 
		VALUES( {$name_inj['values']}, 0 ) ");
}
...
function ostSetOrderStatusToOrder( $orderID, $statusID, $comment = '', $notify = 1, $force = false ){
	...
	if ( (int)$pred_statusID == (int)$statusID && !$force )return;
	...
	if($statusID == CONF_ORDSTATUS_DELIVERED){
