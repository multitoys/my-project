...
//			"classID"			- tax class ID
    function GetProduct($productID)
    {
		...
		';
            ...
            //$product = ZSetSpecialPrice($product);
	...
	SELECT * FROM ".PRODUCTS_TABLE.
        ...
        while ($row = db_fetch_row($q)) {
            ...
            if (($i >= $offset && $i < $offset + $CountRowOnPage) ||
                ...
                $row["PriceWithUnit"] = show_price(ZCalcPrice($row["Price"], $row["SpecialPrice"], $row["skidka"]));
                ...
                $row["PriceWithOutUnit"] = show_priceWithOutUnit(ZCalcPrice($row["Price"], $row["SpecialPrice"], $row["skidka"]));
                $result[] = $row;
            }
            $i++;
        }
...
//					"extraParametrsTemplate"
    function prdSearchProductByTemplate($callBackParam, &$count_row, $navigatorParams = null)
    {
        ...
        while ($_Product = db_fetch_assoc($Result)) {
            ...
            $_Product['PriceWithUnit'] = show_price(ZCalcPrice($_Product["Price"], $_Product["SpecialPrice"], $_Product["skidka"]));
            ...
            $_Product['PriceWithOutUnit'] = show_priceWithOutUnit(ZCalcPrice($_Product["Price"], $_Product["SpecialPrice"], $_Product["skidka"]));
...
// for ($i = 0; $i < count ($Products); $i++){
        // $is_specialoffer = array();
        // foreach($product_lists as $list){
        // $list_items = $list->getProducts(true);
        // foreach ($list_items as $item){
        // if ($item['productID'] == $Products[$i]['productID']){
        // $is_specialoffer[] = $list->id;
        // }
        // }
        // }
        // $Products[$i]['is_specialoffer'] = (count ($is_specialoffer) > 0)? $is_specialoffer : false;
// }
...
// //-----------------------------	
    ...
    { //gets META keywords and description - an HTML code to insert into <head> section
    ...
    function getProductListsByProductID($productID)
    {
        return db_phquery_fetch(DBRFETCH_ASSOC_ALL, '
    SELECT `product_lists`.`name`, `product_lists`.`id`,
      (
        SELECT `productID` IS NOT NULL
        FROM `?#TBL_PRODUCT_LIST_ITEM` `product_list_items`
        WHERE `productID` = ?
          AND `product_list_items`.`list_id` = `product_lists`.`id`
      ) AS `enabled`,
      (
        SELECT COUNT(*)
        FROM `?#TBL_PRODUCT_LIST_ITEM`
        WHERE `list_id` = `product_lists`.`id`
      ) AS `count`
      FROM `?#TBL_PRODUCT_LIST` `product_lists`
  ', $productID);
    }

    function updateProductListsByProductID($product_lists, $productID)
    {
        db_phquery('
    DELETE FROM `?#TBL_PRODUCT_LIST_ITEM`
    WHERE `productID` = ?
  ', $productID);

        foreach ($product_lists as $product_list) {
            db_phquery('
      INSERT INTO `?#TBL_PRODUCT_LIST_ITEM` (`list_id`, `productID`)
      VALUES (?, ?)
