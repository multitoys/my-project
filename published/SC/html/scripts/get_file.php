...
//	require_once(DIR_ROOT.'/smarty/resources/resource.register.php');
    ...
    function TestToken()
    {
        ...
        if (!isset($_SESSION['wbs_username'])) {
            $_SESSION['wbs_username'] = 'YANDEX';
            $_SESSION['wbs_dbkey'] = 'multitoys';
            $_SESSION['LOGIN_PAGE_URL'] = '';
            $_SESSION['SESSION_EXPIRE_PERIOD'] = 300;
            $_SESSION['WBS_ACCESS_SC'] = 1;
            $_SESSION['__WBS_SC_DATA'] = array
            (
                'U_ID'          => 'YANDEX',
                'LANGUAGE_ID'   => 1,
                'LANGUAGE_ISO3' => 'rus',
...
//            {
            ...
            $cs_skidka = (isset($_GET['discount']))?(int)$_SESSION['cs_skidka']:0;
            $cs_skidka_ua = (isset($_GET['discount']))?(int)$_SESSION['cs_skidka_ua']:0;
            $direct_mode = isset($_GET['download'])?false:true;
            $charset = 'windows-1251';
            $fileToDownLoad = DIR_TEMP.'/'.date('Y-m-d').'_'.((int)(date('H') / 4)).'_disc_'.$cs_skidka.'_'.$cs_skidka_ua.'_yandex.xml';
            $fileToDownLoadShortName = "yandex.xml";
            if (!file_exists($fileToDownLoad) || filesize($fileToDownLoad) < 1024 * 1024) {
                $get_discount = '';
                if ($cs_skidka > 0 || $cs_skidka_ua > 0) {
                    $get_discount = '&discount=1';
                }
                $url_path = '/published/SC/html/scripts/?getFileParam=R2V0WWFuZGV4&download=1&did=73'.$get_discount;
                Redirect($url_path);
                //Redirect(set_query('did=73'));
                //include(DIR_ROOT.'/modules/products/market/class.market.php');
                //$market = new Market();
            }
        }
