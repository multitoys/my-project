<?php
    ...
    function db_query($s) //database query
    {
        ...
        if (isset($_GET['debug']) && $_GET['debug'] == 'time') {
            ...
            print '<br /><span onclick="document.getElementById(\'sqldebug_'.$cnt.'\').style.display=document.getElementById(\'sqldebug_'.$cnt.'\').style.display!=\'block\'?\'block\':\'none\'">+</span>'.sprintf('%03d', $cnt).' - <strong>'.$T->timerStop().'</strong> - '.$s;
            ob_start();
            print '<pre>';
            print_r(debug_backtrace());
            print '</pre>';
            $_t = preg_replace('/(\[function\]\s+\=\>\s+)([^\n]+)/msi', '$1<span style=color:red;>$2</span>', ob_get_contents());
            ob_end_clean();
            ?>
            <div id="sqldebug_<?php $cnt; ?>" style="display:none;">
                <?php
                    echo $_t;
                ?>
            </div>
