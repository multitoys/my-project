...
//			objImgLoading.src = this.shop_url+'published/SC/html/scripts/images_common/loading.gif';
	...
	'closeButton': function(parentObject){
		...
		with(this){
			...
			var left = $('#my_frame').position().left+$('#my_frame').width();
			var top = $('#my_frame').position().top-25;

			var style = 'position:absolute; top:'+top+'px; left:'+left+'px; cursor:pointer; cursor:hand';

			setStyle(objCloseButton, style);
	...
	'border': function(parentObj){
		var objBorder = document.getElementById('blck-content');
		if(!objBorder){
