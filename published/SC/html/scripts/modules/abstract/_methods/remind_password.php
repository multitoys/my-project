<?php
    ...
    class RemindPasswordController extends ActionsController
    {
        ...
        function remind()
        {
            ...
            if (!trim($this->getData('email'))) {
                Message::raiseMessageRedirectSQ(MSG_ERROR, '', 'usr_enter_loginemail', '', array('name' => 'error'));
            }
            $res = regSendPasswordToUser($this->getData('email'), $smarty_mail);
            if ($res) {
                Message::raiseMessageRedirectSQ(MSG_SUCCESS, 'ukey=remind_password', 'usr_password_sent', '', array('name' => 'success'));
            } else {
                Message::raiseMessageRedirectSQ(MSG_ERROR, 'ukey=remind_password', 'usr_cant_find_user_in_db', '', array('name' => 'error', 'email' => $this->getData('email')));
            }
        }
