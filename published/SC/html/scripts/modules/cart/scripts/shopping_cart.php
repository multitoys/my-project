...
class ShoppingCartController extends ActionsController{
	...
	function add_product(){
	  ...
	  if (($incart && ($this->getData('force') == 'yes')) || (!$incart))
	  {
	    $variants=array();
...
");
	...
	// Zerg [14.04.13]
	  ...
	  $total_bonus = 0;
	  foreach ($resCart["cart_content"] as $key => $value) {
	  	if(isset($value["Bonus"])) {
	  		$total_bonus += (intval($value["Bonus"])*intval($value["quantity"]));
	  	}
	  }
	  ...
	  $resDiscount = dscGetCartDiscounts( $resCart["total_price"], (isset($_SESSION["log"]) ? $_SESSION["log"] : "") );
		...
		$smarty->assign("bonus_total",	$total_bonus);
