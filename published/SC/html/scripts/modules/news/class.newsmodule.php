...
class News extends ComponentModule {
	...
	function getNews($_ID = null, $CurrentPage = null, $NewsPerPage = null){
		...
		if(!isset($_ID)){
			...
			$News = array();
			$NewsPerPage = isset($NewsPerPage)?intval($NewsPerPage):$this->NewsPerPage;
			$CurrentPage = isset($CurrentPage)?intval($CurrentPage):$this->CurrentPage;
			...
			$sql = 'SELECT * FROM '.$this->DB_TABLE.' WHERE NID='.intval($_ID);
	...
	function sendNews($_ID){
		...
		foreach ($Subscribers as $subscriber){
			if (eregi("^[_\.0-9a-zA-Z-]+@([0-9a-zA-Z-]+\.)+[a-zA-Z]{2,6}$", $subscriber["Email"])){
