...
if ( isset($_GET['search']) || $changeStatusIsPressed || isset($_GET['export_to_excel'])){
	...
	if(isset($_GET['show_all'])){
		...
		$orders = ordGetOrders($callBackParam, $count, array('offset'=>($page-1)*50,'CountRowOnPage'=>(50)));
	}
...
//			),
	...
	if(!$res||$messageClient->getResult('success')===true){
		...
		if ($export_to_excel&&!isset($_GET['search'])) {
			...
			$exportData->sqlQuery = 'SELECT CONCAT(\''.CONF_ORDERID_PREFIX.'\',`orders`.`orderID`) AS orderID,'.
									'`orders`.`order_time`,'.
									'`statuses`.`'.LanguagesManager::ml_getLangFieldName('status_name').'`,'.
									...
									'`orders`.`currency_code`,'.//currency_code
									'(SELECT GROUP_CONCAT(CONCAT(`ord_cont`.`name`,\' x \',`ord_cont`.`Quantity`,\': '.$currencyISO3.' \',ROUND(`ord_cont`.`Quantity`*`ord_cont`.`Price`,2)) ORDER BY `ord_cont`.`itemID` DESC SEPARATOR '."'\n'".') as \'order_content\' FROM '.ORDERED_CARTS_TABLE.' AS `ord_cont` WHERE `ord_cont`.`orderID`=`orders`.`orderID` GROUP BY `ord_cont`.`orderID`) ,'.
									...
									'`orders`.`payment_type`,'.
									'CONCAT(`orders`.`billing_firstname`,\' \',`orders`.`billing_lastname`,'."'\n'".',`orders`.`billing_address`,'."'\n'".','.
