...
class CWebMoney extends PaymentModule {
	...
	function _initVars(){
		...
		parent::_initVars();
		$this->title 		= "WebMoney";
		$this->description 	= "WebMoney Merchant Interface (www.webmoney.ru)<br>ВНИМАНИЕ: После того, как модуль будет установлен, вам необходимо включить опцию приема платежей через Merchant Interface в вашей учетоной записи WebMoney";
		$this->sort_order 	= 0;
		$this->Settings = array( 
	...
	function after_processing_html( $orderID ) 
	{
		...
		$order_amount = round($order_amount/((float)$this->_getSettingValue('CONF_PAYMENTMODULE_WEBMONEY_MERCHANT_EXCHANGERATE')),2);
		...
		$res .= 
		    ...
		    "	<input type='hidden' name='LMI_SUCCESS_URL' value='".getTransactionResultURL('success')."'>".
			"	<input type='hidden' name='LMI_FAIL_URL' value='".getTransactionResultURL('failure')."'>".
			...
			if($this->validateResultKey(array($orderID,$order['customer_email']))){
				...
				if($sharedSecret){//check callback sign
					...
					if($transaction_result == 'success'){
						...
						$this->LogFile = DIR_TEMP.'/webmoney.log';
						$this->_log(LOGMODE_MSG,$comment."\r\nПараметры неверного запроса:\r\n".var_export(array('GET'=>$_GET,'POST'=>$_POST,'REQUEST'=>$_REQUEST),true));
						ostSetOrderStatusToOrder( $orderID, $statusID,$comment,0,true);
