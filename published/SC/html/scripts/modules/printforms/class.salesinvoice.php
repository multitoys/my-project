<?php
/**
 * @connect_module_class_name SalesInvoice
 ...
 * @_side backend
 * @_name Счет-фактура
 * @_description Для использования на территории РФ. Счет-фактуру может распечатать только администратор интернет-магазина.
 */
class SalesInvoice extends Forms
{
	function _initVars()
	{
		parent::_initVars();
	...
	function _initSettingFields()
	{
		...
		$this->SettingsFields['IP_NAME'] = array(
		'settings_value' 		=> '',
		'settings_title' 			=> 'Индивидуальный предприниматель',
		'settings_description' 	=> 'Укажите Фамилию И.О.',
		'settings_html_function' 	=> 'text',
		'sort_order' 			=> 1,
		);
		$this->SettingsFields['IP_REGISTRATION'] = array(
		'settings_value' 		=> '',
		'settings_title' 			=> 'Реквизиты',
		'settings_description' 	=> 'Укажите реквизиты свидетельства о государственной регистрации индивидуального предпринимателя',
		'settings_html_function' 	=> 'textarea',
		'sort_order' 			=> 1,
		);
		...
		$this->SettingsFields['CUSTOMER_INN_FIELD'] = array(
		'settings_value' 		=> false,
		'settings_title' 			=> 'ИНН покупателя',
		'settings_description' 	=> 'Поле "ИНН" в форме регистрации',
		'settings_html_function' 	=> 'select Forms::select_customer_fields',
		'sort_order' 			=> 1,
		);
		$this->SettingsFields['CUSTOMER_KPP_FIELD'] = array(
		'settings_value' 		=> false,
		'settings_title' 			=> 'КПП покупателя',
		'settings_description' 	=> 'Поле "КПП" в форме регистрации',
		'settings_html_function' 	=> 'select Forms::select_customer_fields',
		'sort_order' 			=> 1,
		);
	}
	...
	function display($strict = true)
	{
		...
		if($order){
			...
			foreach($order['reg_fields_values'] as $customer_field){
				switch($customer_field['reg_field_ID']){
					...
					case $this->CUSTOMER_PHONE_FIELD:
						...
						break;
					case $this->CUSTOMER_INN_FIELD:
						$customer['inn'] = $customer_field['reg_field_value'];
						break;
					case $this->CUSTOMER_KPP_FIELD:
						$customer['kpp'] = $customer_field['reg_field_value'];
						break;
			...
			$order['shipping_cost'] *=$currency_rate;// /$order['currency_value'];
			...
			//var_dump(array('RUR rate'=>$currency_rate,'Order Rate'=>$order['currency_value'],'discount'=>$discount,$order['order_discount'],$order['clear_total_price']));
			foreach($order_content as &$order_item){
				//var_dump(array('itemPrice'=>$order_item['ItemPrice'],'price'=>$order_item['Price'],'qty'=>$order_item['Quantity']));
				...
				//var_dump(array('itemPrice'=>$order_item['ItemPrice'],'price'=>$order_item['Price']));
				//var_dump($order_item);
				if($this->NDS){
				...
				//var_dump(array('itemPrice'=>$order_item['ItemPrice'],'price'=>$order_item['Price'],'qty'=>$order_item['Quantity'],'nds'=>$order_item['Nds_ItemPrice']));
			...
			//var_dump(array($order['order_discount'],$order['total_price'],$order['total_nds_price'],$order['clear_total_price']),$order_content);
			//$order['clear_total_price'] = $currency_rate*$order['clear_total_price'];
			...
			$order['total_nds_price'] = round($order['total_nds_price']*$delta+$order['shipping_cost'],2);
			$order['total_price'] = round($order['total_price']*$delta+$order['shipping_cost'],2);
			/*var_dump(array($currency_rate,$discount,
			 'clear_total_price'=>$order['clear_total_price'],
			 'total_price'=>$order['total_price'],
			 'total_nds_price'=>$order['total_nds_price'],
			 'delta'=>$delta,$order['total_price']-$order['clear_total_price']));*/
