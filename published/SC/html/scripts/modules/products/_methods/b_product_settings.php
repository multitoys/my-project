...
class prdsetActions extends ActionsController {
	...
	function save_product(){
		...
		do{
			...
			if(!$isDownloadable&&$productEntry->eproduct_filename){
				$res = Functions::exec('file_remove', array(DIR_PRODUCTS_FILES.'/'.$productEntry->eproduct_filename));
				if(PEAR::isError($res))break;
			...
			updateProductListsByProductID($this->getData('product_lists'), $productID);
	...
	function main(){
		...
		$smarty->assign('product_lists', getProductListsByProductID($productID));
