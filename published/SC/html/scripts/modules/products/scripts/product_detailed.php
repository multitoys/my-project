<?php
    ...
    if (isset($_POST["add2cart_x"])) { //add product to cart

        $variants = array();
        foreach ($_POST as $key => $val) {

            if (false === strpos($key, 'option_')) continue;
            $variants[] = $val;
        }

        $qty = max(1, isset($_POST['product_qty']) ? (int)$_POST['product_qty'] : 1);
...
//		$Register = &Register::getInstance();
    ...
    if (isset($_GET["vote"], $productID)) //vote for a product
    {
    ...
    if (isset($_POST["request_information"])) //email inquiry to administrator
    {
        ...
        if (trim($customer_email) != "" && trim($customer_name) != "" && trim($message_subject) != "" && trim($message_text) != "" && eregi("^[_\.0-9a-zA-Z-]+@([0-9a-zA-Z-]+\.)+[a-zA-Z]{2,6}$", $customer_email)) {
            ...
            /*$mailer = new SSMailer();
            $mailer->From = $customer_email;
            $mailer->FromName = $customer_name;
            
            $mailer->AddAddress(CONF_GENERAL_EMAIL);
            $mailer->Subject = $message_subject;
            $mailer->Body = $message_text;
...
//            $smarty->assign("product_related_number", count($related));
            ...
            */
            ...
            if (!file_exists(DIR_PRODUCTS_PICTURES . "/" . $a["picture"])) $a["picture"] = '';
            if (!file_exists(DIR_PRODUCTS_PICTURES . "/" . $a["thumbnail"])) $a["thumbnail"] = '';
            if (!file_exists(DIR_PRODUCTS_PICTURES . "/" . $a["big_picture"])) $a["big_picture"] = '';
            ...
            if (Message::isMessage($Message) && $Message->is_set() && isset($Message->prd_request)) $smarty->assign('prd_request', $Message->prd_request);
            ...
            $smarty->assign('vote_completed', isset($_SESSION["vote_completed"][$productID]) ? 1 : 0);
        }
    }
