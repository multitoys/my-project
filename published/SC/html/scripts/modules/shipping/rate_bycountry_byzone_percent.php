...
*/
define('CSHIPPINGMODULEBYCOUNTRYBYZONEPERCENT_TABLE', DBTABLE_PREFIX.'_module_shipping_bycountries_byzones_rates_percent');
...
function settingCONF_BYCOUNTRY_BYZONE_PERCENT_FORM($module_id)
{
	...
	if(!$module_id)$module_id = isset($_GET['setting_up'])?$_GET['setting_up']:0;
...
class CShippingModuleByCountryByZonePercent extends ShippingRateCalculator{
	...
	function _initSettingFields()
	{
		$this->SettingsFields['CONF_SHIPPING_MODULE_BYCOUNTRYBYZONEPERCENT_IS_INSTALLED'] = array(
			...
			'settings_html_function' 	=> 'settingCONF_BYCOUNTRY_BYZONE_PERCENT_FORM('.$this->ModuleConfigID.')', 
			'sort_order' 			=> 1,
		);
	}
	...
	function calculate_shipping_rate($order, $address){
		...
		return RoundFloatValue( $order["order_amount"] * $percent / 100 );
	}
	...
	function install(){
		...
		if(!db_table_exists(CSHIPPINGMODULEBYCOUNTRYBYZONEPERCENT_TABLE)){
