...
if ( isset($categoryID) /*&& isset($_GET["search_with_change_category_ability"])*/ &&  isset($_GET["advanced_search_in_category"]) ){
	...
	function _sortSetting( &$smarty )
	{
		...
		foreach ($sort_fields as $field){
			...
			$sort_string = str_replace( "{ASC_".$field[1]."}", $field[0] == $current_sort_field && $current_sort_direction == 'ASC'?translate("str_ascending"):"<a href='".set_query("&sort={$field[0]}&direction=ASC")."'>".translate("str_ascending")."</a>",	$sort_string );
			$sort_string = str_replace( "{DESC_".$field[1]."}", $field[0] == $current_sort_field && $current_sort_direction == 'DESC'?translate("str_descending"):"<a href='".set_query("&sort={$field[0]}&direction=DESC")."'>".translate("str_descending")."</a>",	$sort_string );
		}
