        ...
        class Test extends ComponentModule
        {
            ...
            function initInterfaces()
            {
                ...
                $this->Interfaces = array(
                    ...
                    'b_db_synchronization'     => array(
                        'name' => 'Синхронизация баз данных',
                    ),
                ...
                $this->__registerComponent('logo', 'cpt_lbl_logo', array(TPLID_GENERAL_LAYOUT), null, array('file' => array('type' => 'image_file', 'params' => array('name' => 'file', 'current_file_title' => 'Current logo image:', 'upload_file_title' => 'Upload new logo:', 'value' => 'logo.png'))));
            ...
            function methodFOrderDetailed()
            {
                ...
                if (!isset($_SESSION['log']) || !$_SESSION['log']) {
                    ...
                    $customer_last_name = isset($_POST['customer_last_name']) ? $_POST['customer_last_name'] : (isset($_GET['customer_last_name']) ? $_GET['customer_last_name'] : '');;
                    ...
                    if ($order) {
                        if ((mb_strtolower($order["customer_lastname"], 'UTF-8') == mb_strtolower($customer_last_name, 'UTF-8'))
                            ||
                            (md5(mb_strtolower($order["customer_lastname"], 'UTF-8')) == (isset($_GET['hash']) ? $_GET['hash'] : ''))
                        ) //||(mb_strtolower($order["customer_firstname"],'UTF-8') != mb_strtolower($customer_first_name,'UTF-8'))
                        {
                ...
                if ($order) {
                    ...
                    //$order_status_report = xNl2Br(stGetOrderStatusReport( $orderID ), 'status_comment');
                    ...
                    if ($smarty->get_template_vars('full_info') == 1) {
                        ...
                        if (($currentPaymentModule != null) && ($order['statusID'] != CONF_ORDSTATUS_DELIVERED)) {
                        ...
                        /*@var $currentShippingModule ShippingRateCalculator*/
                        if ($currentShippingModule != null) {
                            $shipping_info = '';//$currentShippingModule-> after_processing_html($_GET['orderID']);
                        } else {
                            $shipping_info = '';
                        }
                        ...
                        //'language'=>$_language,
                        $properties = array('language' => $_language, 'side' => 'frontend');
                        $print_forms = array_merge(
                            Forms::listConnectedModules(array('type' => Forms::GENERIC_FORM, 'language' => $_language, 'side' => 'frontend')),
                            $currentPaymentModule ? Forms::listConnectedModules(array('type' => Forms::MODULE_FORM, 'sub_type' => $currentPaymentModule->getConnectedPrintforms(), 'side' => 'frontend')) : array(),
                            $currentShippingModule ? Forms::listConnectedModules(array('type' => Forms::MODULE_FORM, 'sub_type' => $currentShippingModule->getConnectedPrintforms(), 'side' => 'frontend')) : array()
                        );
            ...
            function methodOrdersHistory()
            {
                ...
                $navigatorHtml = GetNavigatorHtml('', 100,
            ...
            function method_logout()
            {
                ...
                // unset($_SESSION['cs_first_name']);
                // unset($_SESSION['cs_last_name']);
                // unset($_SESSION['cs_may_order_until']);
                // unset($_SESSION['cs_skidka']);
                // unset($_SESSION['cs_special_price']);
