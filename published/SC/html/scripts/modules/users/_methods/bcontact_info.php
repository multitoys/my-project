...
class EditCustomerContactInfo extends ActionsController
{
    ...
    function main()
    {
        ...
        */
        ...
        // check right to unlimited access
        $user = $_SESSION["wbs_username"];
        $query = "SELECT AR_VALUE FROM U_ACCESSRIGHTS WHERE AR_ID='".$user."' AND AR_OBJECT_ID='SC__226'";  
        $access = mysql_fetch_object(mysql_query($query))->AR_VALUE;
        if($access && intval($access) == 1) $enable_access = 1; else $enable_access = 0;
        ...
        $smarty->assign('is_right', $enable_access);
