(function () {
    ...
    function isEmpty(d, e, f) {
        ...
        while (n = w.nextNode()) {
            ...
            if (n.nodeType == 3 && n.nodeValue && /[^\s\u00a0]+/.test(n.nodeValue))return false;
            if (n.nodeType == 1 && /^(HR|IMG|TABLE)$/.test(n.nodeName))return false;
        }
        return true;
    };
    tinymce.create('tinymce.plugins.Safari', {
        init: function (ed) {
            ...
            ed.addCommand('CreateLink', function (u, v) {
                ...
                } else ed.getDoc().execCommand("CreateLink", false, v);
            });
            ...
            ed.addCommand('FormatBlock', function (u, v) {
                ...
                if (e)dom.replace(dom.create(v), e, 1); else ed.getDoc().execCommand("FormatBlock", false, v);
            });
            ...
            ed.onKeyPress.add(function (ed, e) {
                ...
                if (e.keyCode == 13) {
                    ...
                    if (li = dom.getParent(se, 'LI')) {
                        ...
                        if (isEmpty(doc, li)) {
                            if (n = dom.getParent(lic.parentNode, 'LI,OL,UL'))return;
                            ...
                            be = r1.cloneContents();
                            af = r2.cloneContents();
                            if (!isEmpty(doc, af))dom.insertAfter(af, n);
                            dom.insertAfter(pa, n);
                            if (!isEmpty(doc, be))dom.insertAfter(be, n);
                            dom.remove(n);
                            n = pa.firstChild;
                            r1 = doc.createRange();
                            r1.setStartBefore(n);
            ...
            ed.onExecCommand.add(function (ed, cmd) {
                ...
                if (cmd == 'InsertUnorderedList' || cmd == 'InsertOrderedList') {
                    sel = ed.selection;
                    dom = ed.dom;
                    if (bl = dom.getParent(sel.getNode(), function (n) {
                            return /^(H[1-6]|P|ADDRESS|PRE)$/.test(n.nodeName);
                        })) {
                        bm = sel.getBookmark();
                        dom.remove(bl, 1);
            ...
            ed.onInit.add(function () {
                t._fixWebKitSpans();
                if (isOldWebKit)t._patchSafari2x(ed);
            });
            ed.onSetContent.add(function () {
                ...
                each(['strong', 'b', 'em', 'u', 'strike', 'sub', 'sup', 'a'], function (v) {
                    each(grep(dom.select(v)).reverse(), function (n) {
                        ...
                        if (nn == 'a') {
                            if (n.name)dom.replace(dom.create('img', {
                                mce_name: 'a',
                                name: n.name,
                                'class': 'mceItemAnchor'
                            }), n);
                    ...
                    if (o.get) {
                        if (dom.hasClass(n, 'Apple-style-span')) {
                            ...
                            switch (dom.getAttrib(n, 'mce_name')) {
                                case'font':
                                    if (!ed.settings.convert_fonts_to_spans)dom.setAttrib(n, 'style', '');
                                    break;
                                case'strong':
                                case'em':
                                case'sub':
                                ...
                                case'strike':
                                case'u':
                                    if (!ed.settings.inline_styles)dom.setAttrib(n, 'style', ''); else dom.setAttrib(n, 'mce_name', '');
                                    break;
                                default:
                                    if (!ed.settings.inline_styles)dom.setAttrib(n, 'style', '');
                            }
                            if (bg)n.style.backgroundColor = bg;
                        }
                    }
                    if (dom.hasClass(n, 'mceItemRemoved'))dom.remove(n, 1);
                });
            });
            ...
            if (!isOldWebKit) {
                Event.add(ed.getDoc(), 'DOMNodeInserted', function (e) {
                    e = e.target;
                    if (e && e.nodeType == 1)t._fixAppleSpan(e);
                });
            } else {
            ...
            if (dom.getAttrib(e, 'mce_fixed'))return;
            if (e.nodeName == 'SPAN' && e.className == 'Apple-style-span') {
                ...
                if (!s.convert_fonts_to_spans) {
                    ...
                    if (st.fontSize)dom.setStyle(e, 'fontSize', fzn[inArray(fz, st.fontSize)]);
                }
                if (st.fontWeight == 'bold')dom.setAttrib(e, 'mce_name', 'strong');
                if (st.fontStyle == 'italic')dom.setAttrib(e, 'mce_name', 'em');
                if (st.textDecoration == 'underline')dom.setAttrib(e, 'mce_name', 'u');
                if (st.textDecoration == 'line-through')dom.setAttrib(e, 'mce_name', 'strike');
                if (st.verticalAlign == 'super')dom.setAttrib(e, 'mce_name', 'sup');
                if (st.verticalAlign == 'sub')dom.setAttrib(e, 'mce_name', 'sub');
