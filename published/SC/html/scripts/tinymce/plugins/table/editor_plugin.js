        ...
        init: function (ed, url) {
            ...
            ed.onInit.add(function () {
                if (ed && ed.plugins.contextmenu) {
                    ed.plugins.contextmenu.onContextMenu.add(function (th, m, e) {
                        ...
                        } else m.add({title: 'table.desc', icon: 'table', cmd: 'mceInsertTable', ui: true});
                    });
                }
            });
            ...
            if (!tinymce.isIE) {
                if (ed.getParam('table_selection', true)) {
                    ed.onClick.add(function (ed, e) {
                        e = e.target;
                        if (e.nodeName === 'TABLE')ed.selection.select(e);
                    });
                }
            }
            ...
            if (!tinymce.isIE) {
                ed.onBeforeSetContent.add(function (ed, o) {
                    if (o.initial)o.content = o.content.replace(/<(td|th)([^>]+|)>\s*<\/(td|th)>/g, tinymce.isOpera ? '<$1$2>&nbsp;</$1>' : '<$1$2><br mce_bogus="1" /></$1>');
                });
            }
            ...
            if (trElm && tdElm == null)tdElm = trElm.cells[0];
            function inArray(ar, v) {
                for (var i = 0; i < ar.length; i++) {
                    if (ar[i].length > 0 && inArray(ar[i], v))return true;
            ...
            function makeTD() {
                ...
                if (!tinymce.isIE)newTD.innerHTML = '<br mce_bogus="1"/>';
            }
            ...
            function getNextCell(table, cell) {
                ...
                for (i = 0; i < table.rows.length; i++)for (j = 0; j < table.rows[i].cells.length; j++, x++)cells[x] = table.rows[i].cells[j];
                for (i = 0; i < cells.length; i++)if (cells[i] == cell)if (nextCell = cells[i + 1])return nextCell;
            }
                ...
                if (new_tr.cells.length != tr.childNodes.length) {
                    ...
                    for (var x = 0; td = getCell(grid, cpos.rowindex, x); x++) {
                        ...
                        if (inArray(cells, td)) {
                            ...
                            for (var i = x; i < x + td.colSpan; i++)new_tr.childNodes[i]._delete = true;
                        }
                        if ((lastElm == null || td != lastElm) && sd.rowspan > 1)td.rowSpan = sd.rowspan + 1;
                        lastElm = td;
                    }
            ...
            function addRows(td_elm, tr_elm, rowspan) {
                ...
                for (var i = 1; i < rowspan && trNext; i++) {
                    var newTD = doc.createElement("td");
                    if (!tinymce.isIE)newTD.innerHTML = '<br mce_bogus="1"/>';
                    if (tinymce.isIE)trNext.insertBefore(newTD, trNext.cells(td_elm.cellIndex)); else trNext.insertBefore(newTD, trNext.cells[td_elm.cellIndex]);
            ...
            switch (command) {
                ...
                case"mceInsertTable":
                    if (user_interface) {
                        inst.windowManager.open({
                            ...
                            inline: 1
                        }, {plugin_url: url, action: value ? value.action : 0});
                    }
                    return true;
                case"mceTableDelete":
                ...
                case"mceTablePasteRowBefore":
                case"mceTablePasteRowAfter":
                    if (!tableElm)return true;
                    if (trElm && tableElm != trElm.parentNode)tableElm = trElm.parentNode;
                    if (tableElm && trElm) {
                        switch (command) {
                            case"mceTableCutRow":
                                if (!trElm || !tdElm)return true;
                            ...
                            case"mceTablePasteRowBefore":
                                ...
                                if (prevTR != null)trimRow(tableElm, prevTR, prevTR.cells[0], newTR);
                            ...
                            case"mceTablePasteRowAfter":
                                ...
                                if (nextTR == null)trElm.parentNode.appendChild(newTR); else nextTR.parentNode.insertBefore(newTR, nextTR);
                                break;
                            case"mceTableInsertRowBefore":
                                if (!trElm || !tdElm)return true;
                                ...
                                var lastTDElm = null;
                                cpos.rowindex--;
                                if (cpos.rowindex < 0)cpos.rowindex = 0;
                                for (var x = 0; tdElm = getCell(grid, cpos.rowindex, x); x++) {
                                    if (tdElm != lastTDElm) {
                                        ...
                                        if (sd['rowspan'] == 1) {
                                            ...
                                            newTR.appendChild(newTD);
                                        } else tdElm.rowSpan = sd['rowspan'] + 1;
                                        lastTDElm = tdElm;
                                    }
                                }
                                trElm.parentNode.insertBefore(newTR, trElm);
                            ...
                            case"mceTableInsertRowAfter":
                                ...
                                if (newTR.hasChildNodes()) {
                                    var nextTR = nextElm(trElm, "TR");
                                    if (nextTR)nextTR.parentNode.insertBefore(newTR, nextTR); else tableElm.appendChild(newTR);
                                }
                                select(0, 1);
                                break;
                            case"mceTableDeleteRow":
                                ...
                                for (var x = 0; x < cells.length; x++) {
                                    if (cells[x].rowSpan > 1) {
                                        ...
                                        if (nextTD == null)nextTR.appendChild(newTD); else nextTR.insertBefore(newTD, nextTD);
                                    }
                                }
                                var lastTDElm = null;
                                for (var x = 0; tdElm = getCell(grid, cpos.rowindex, x); x++) {
                                    if (tdElm != lastTDElm) {
                                        ...
                                        if (sd.rowspan > 1) {
                                            ...
                                            if (trElm.parentNode)trElm._delete = true;
                                        }
                                        lastTDElm = tdElm;
                                    }
                                }
                                ...
                                for (var y = 0; tdElm = getCell(grid, y, cpos.cellindex); y++) {
                                    if (tdElm != lastTDElm) {
                                        ...
                                        if (sd['colspan'] == 1) {
                                            ...
                                            if (nextTD == null)tdElm.parentNode.appendChild(newTD); else nextTD.parentNode.insertBefore(newTD, nextTD);
                                        } else tdElm.colSpan++;
                                        lastTDElm = tdElm;
                                    }
                                }
                            ...
                            case"mceTableDeleteCol":
                                ...
                                for (var y = 0; tdElm = getCell(grid, y, cpos.cellindex); y++) {
                                    if (tdElm != lastTDElm) {
                                        var sd = getColRowSpan(tdElm);
                                        if (sd['colspan'] > 1)tdElm.colSpan = sd['colspan'] - 1; else {
                                            if (tdElm.parentNode)tdElm.parentNode.removeChild(tdElm);
                                        }
                                        lastTDElm = tdElm;
                                    }
                                }
                            ...
                            case"mceTableSplitCells":
                                ...
                                if (colspan > 1 || rowspan > 1) {
                                    ...
                                    for (var i = 1; i < colspan; i++) {
                                        ...
                                        if (rowspan > 1)addRows(newTD, trElm, rowspan);
                                    }
                            ...
                            case"mceTableMergeCells":
                                ...
                                if (tinymce.isIE || sel.rangeCount == 1) {
                                    if (user_interface) {
                                        ...
                                        var cpos = getCellPos(grid, tdElm);
                                        if (("" + numRows) == "NaN")numRows = 1;
                                        if (("" + numCols) == "NaN")numCols = 1;
                                        ...
                                        for (var y = cpos.rowindex; y < grid.length; y++) {
                                            ...
                                            for (var x = cpos.cellindex; x < grid[y].length; x++) {
                                                ...
                                                if (td && !inArray(rows, td) && !inArray(rowCells, td)) {
                                                    ...
                                                    if (cp.cellindex < cpos.cellindex + numCols && cp.rowindex < cpos.rowindex + numRows)rowCells[rowCells.length] = td;
                                                }
                                            }
                                            if (rowCells.length > 0)rows[rows.length] = rowCells;
                                            var td = getCell(grid, cpos.rowindex, cpos.cellindex);
                                            each(ed.dom.select('br', td), function (e, i) {
                                                if (i > 0 && ed.dom.getAttrib('mce_bogus'))ed.dom.remove(e);
                                            });
                                        }
                                    }
                                } else {
                                    ...
                                    var curRow = null;
                                    var x1 = -1, y1 = -1, x2, y2;
                                    if (sel.rangeCount < 2)return true;
                                    for (var i = 0; i < sel.rangeCount; i++) {
                                        var rng = sel.getRangeAt(i);
                                        var tdElm = rng.startContainer.childNodes[rng.startOffset];
                                        if (!tdElm)break;
                                        if (tdElm.nodeName == "TD" || tdElm.nodeName == "TH")cells[cells.length] = tdElm;
                                    }
                                    var tRows = tableElm.rows;
                                ...
                                for (var y = 0; y < rows.length; y++) {
                                    ...
                                    if (rowColSpan > colSpan)colSpan = rowColSpan;
                                    lastRowSpan = -1;
                                }
                                var lastColSpan = -1;
                                for (var x = 0; x < rows[0].length; x++) {
                                    ...
                                    if (colRowSpan > rowSpan)rowSpan = colRowSpan;
                                    lastColSpan = -1;
                                }
                                tdElm = rows[0][0];
                                ...
                                for (var y = 0; y < rows.length; y++) {
                                    for (var x = 0; x < rows[y].length; x++) {
                                        ...
                                        if (chk != "<br/>" && chk != "<br>" && chk != '<br mce_bogus="1"/>' && (x + y > 0))tdElm.innerHTML += html;
                                        if (rows[y][x] != tdElm && !rows[y][x]._deleted) {
                                            ...
                                            if (!tr.hasChildNodes()) {
                                                ...
                                                for (var x = 0; cellElm = getCell(grid, cpos.rowindex, x); x++) {
                                                    if (cellElm != lastCell && cellElm.rowSpan > 1)cellElm.rowSpan--;
                                                    lastCell = cellElm;
                                                }
                                                if (tdElm.rowSpan > 1)tdElm.rowSpan--;
                                            }
                                        }
                                    }
                                }
                                each(ed.dom.select('br', tdElm), function (e, i) {
                                    if (i > 0 && ed.dom.getAttrib(e, 'mce_bogus'))ed.dom.remove(e);
                                });
                                break;
                        }
                        tableElm = inst.dom.getParent(inst.selection.getNode(), "table");
