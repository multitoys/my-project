    ...
    tinymce.create('tinymce.plugins.ToolbarToggle', {
        init: function (ed, url) {
            ed.onPostRender.add(function (ed, cm) {
                ...
                setTimeout(function () {
                    ed.execCommand('mceToolbarToggle', true, null)
                }, 100)
            });
            ed.addCommand('mceToolbarToggle', function () {
                try {
                    ...
                    while (toolbar = document.getElementById(id + '_toolbar' + (++i))) {
                        toolbar.style.display = ed.settings.plugin_toolbartoggle_show ? '' : 'none'
                    }
                    var editorTable = document.getElementById(id + '_ifr');
                    if (editorTable && editorTable.parentNode.offsetHeight) {
                        editorTable.style.height = '' + editorTable.parentNode.offsetHeight + 'px'
                    } else if (editorTable) {
            ...
            ed.onNodeChange.add(function (ed, cm) {
                cm.setActive('toolbartoggle', ed.getParam('plugin_toolbartoggle_show'))
            })
            ...
            return {
                ...
                author: 'WebAsyst',
                authorurl: 'http://webasyst.net',
                infourl: 'http://www.webasyst.net/shop/',
                version: "1.0"
            }
        }
    });
