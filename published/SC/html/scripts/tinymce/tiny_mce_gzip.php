<?php
    ...
    $cachePath = DIR_TEMP;//realpath("."); // Cache path, this is where the .gz files will be stored
	...
	*/);
    ...
    if ($diskCache) {
        ...
        if ($compress)
            $cacheFile = $cachePath."/.cache.tiny_mce_".$cacheKey.".gz";
        else
            $cacheFile = $cachePath."/.cache.tiny_mce_".$cacheKey.".js";
    }
    ...
    if ($diskCache && $supportsGzip && file_exists($cacheFile)) {
        $mtime = max($mtime, filemtime($cacheFile));
