...
//require_once( WBS_DIR."/published/SC/sc_dbfunctions_cmn.php" );
    ...
    if (isset($_GET['debug'])) {
        print "<pre>" . htmlentities(var_export(ApplicationlimitExceeded('SC', 'eng', HOST_SC_FIRST_REST), false)) . "</pre>";
        exit;
    }
...
//
    ...
    if (strlen($session_name) && ($session_name == ini_get('session.name') . '_SC')) {
