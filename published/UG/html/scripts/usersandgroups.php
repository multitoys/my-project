<?php
	...
	pageUserAuthorization( $SCR_ID, "UG", false);
	...
	switch (true) {
		case true :
			...
			if ( isset($_COOKIE['splitterView'.$AA_APP_ID.$currentUser]) )
				$treePanelWidth = (int)$_COOKIE['splitterView'.$UG_APP_ID.$currentUser];
			else
				$treePanelWidth = 200;
			$treePanelHide = (@$_COOKIE['splitterVisible'.$UG_APP_ID.$currentUser] == "false");
			...
			$viewMenu[$kernelStrings['cm_gridview_menu']] = sprintf( $processAjaxButtonTemplate, 'gridviewbtn' )."||null||$checked";

			$checked = ($viewMode == UL_LIST_VIEW) ? "checked" : "unchecked";
			$viewMenu[$kernelStrings['cm_listview_menu']] = sprintf( $processAjaxButtonTemplate, 'listviewbtn' )."||null||$checked";

			$viewMenu[$kernelStrings['cm_custview_menu']] = sprintf( $processButtonTemplate, 'custviewbtn' );
			
			// Prepare the Reports Menu
			
			//$canReports = checkUserFunctionsRights( $currentUser, $AA_APP_ID, APP_CANREPORTS_RIGHTS, $kernelStrings );
			
			$reportsMenu[$kernelStrings['cm_rp_accessrightsusers_menu']] = prepareURLStr(PAGE_AA_ACCESSRIGHTS_USERS, array ());
			$reportsMenu[$kernelStrings['cm_rp_accessrightsgroups_menu']] = prepareURLStr(PAGE_AA_ACCESSRIGHTS_GROUPS, array ());
	...
	$preproc = new php_preprocessor( $templateName, $kernelStrings, $language, "UG");
	if ($searchString != "")
		$preproc->assign( PAGE_TITLE, $kernelStrings['cm_searchresult_title'] );
	else
	...
	if ($preproc->get_template_vars('ajaxAccess')) {
		...
		$ajaxRes = array ();
		$ajaxRes["toolbar"] = simple_ajax_get_toolbar ("uglist_toolbar.htm", $preproc);
		$ajaxRes["rightContent"] = $preproc->fetch( "uglist_rightpanel.htm" );
