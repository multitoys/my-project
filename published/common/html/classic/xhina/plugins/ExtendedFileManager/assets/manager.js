    ...
    if (manager_mode == 'image' && typeof colorPicker != "undefined" && document.getElementById('bgCol_pick')) {
        ...
        var bgColPicker = new colorPicker({
            cellsize: '5px', callback: function (color) {
                f_backgroundColor.value = color;
            }
        });
        bgCol_pick.onclick = function () {
            bgColPicker.open('top,right', f_backgroundColor);
        }

        var bdCol_pick = document.getElementById('bdCol_pick');
        var f_borderColor = document.getElementById('f_borderColor');
        var bdColPicker = new colorPicker({
            cellsize: '5px', callback: function (color) {
                f_borderColor.value = color;
            }
        });
        bdCol_pick.onclick = function () {
            bdColPicker.open('top,right', f_borderColor);
        }
    }
