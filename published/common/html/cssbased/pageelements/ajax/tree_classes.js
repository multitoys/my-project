function TreeDDManager() {
    ...
    this.copyMoveNode = function (action, tree, node, oldParent, newParent) {
        ...
        AjaxLoader.doRequest(this.config.nodeMovedUrl, this.onNodeMovedHandler, {
            action: action,
            folderID: node.id,
    ...
    this.onNodeMovedHandler = function (response, options) {
        ...
        if (result.success) {
            ...
            newNode = new nodeClass({
                id: result.newID,
                text: movedNode.text,
                link: this.config.linkPrefix + result.encNewID,
                iconCls: iconCls,
                allowDrag: movedNode.attributes.allowDrag,
                allowDrop: movedNode.attributes.allowDrop,
                editable: movedNode.attributes.editable
            });
...
copyMoveDlg.onAfterInit = function () {
    ...
    this.dialog.addButton(foldersTree.config.strings.cancel, function () {
        this.hide()
    }, this.dialog);
}