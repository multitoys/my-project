...
function UpdateSplitterPanels() {
    ...
    //if (SplitterInfo.LeftPanelVisible)
    if (SplitterInfo.LeftPanelContainer != null) {
        rightPanelWidth -= 6;
    }
...
function InitSplitter() {
    ...
    //
    if (document.getElementById("SplitterShowLeftPanelBtn") != null && document.getElementById("SplitterShowLeftPanelBtn").style.display != "none") {
        SplitterInfo.LeftPanelVisible = false;
        SplitterHideLeftPanel();
    }
...
function WriteSplitterCookie(value, type) {
    if (type == null)
        type = "splitterView";
    var date = new Date();
    var name = type + document.splitterName;
...
function ReadSplitterCookie(type) {
    if (type == null)
        type = "splitterView";
...
function SplitterChangeRightHeader(newHeader) {
    document.getElementById("RightPanelHeader").getElementsByTagName("td")[0].innerHTML = newHeader;
}

function SplitterChangeRightCaption(newCaption) {
    document.getElementById("RightPanelHeaderCaption").innerHTML = newCaption;
}

function SplitterHideLeftPanel() {
    document.getElementById("SplitterLeftPanelContainer").style.display = "none";
    WriteSplitterCookie(false, "splitterVisible");
    if (document.getElementById("SplitterShowLeftPanelBtn"))
        document.getElementById("SplitterShowLeftPanelBtn").style.display = "block";
    SplitterInfo.LeftPanelVisible = false;
    UpdateSplitterPanels();
    return false;
}

function SplitterShowLeftPanel() {
    WriteSplitterCookie(true, "splitterVisible");
    document.getElementById("SplitterLeftPanelContainer").style.display = (SplitterInfo.IsIe) ? "block" : "table-cell";
    if (SplitterInfo.LeftPanelWidth != null)
        document.getElementById("SplitterLeftPanelContainer").style.width = SplitterInfo.LeftPanelWidth;
    if (document.getElementById("SplitterShowLeftPanelBtn"))
        document.getElementById("SplitterShowLeftPanelBtn").style.display = "none";
    SplitterInfo.LeftPanelVisible = true;
    UpdateSplitterPanels();
    return false;
}