<?php
    ...
    class Subsys_JsHttpRequest_Php
    {
        ...
        function Subsys_JsHttpRequest_Php($enc)
        {
            ...
            $this->LOADER = "SCRIPT";
            if (preg_match('/(\d+)((?:-\w+)?)$/us', $_SERVER['QUERY_STRING'], $m)) {
                $this->SCRIPT_ID = $m[1];
        ...
        {
            // Parse encoding.
            preg_match('/^(\S*)(?:\s+(\S*))$/u', $enc, $p);
            $this->SCRIPT_ENCODING = strtolower(isset($p[1]) && $p[1] ? $p[1] : $enc);
            $this->SCRIPT_DECODE_MODE = isset($p[2]) && $p[2] ? $p[2] : '';
        ...
        function _correctQueryString()
        {
            ...
            foreach (array('_GET' => $_SERVER['QUERY_STRING'], '_POST' => @$GLOBALS['HTTP_RAW_POST_DATA']) as $dst => $src) {
                if (isset($GLOBALS[$dst])) {
                    ...
                    $s = preg_replace('/%(?!5B)(?!5D)([0-9a-f]{2})/usi', '%u00\\1', $src);
        ...
        {
            // Check for error.
            if (preg_match('{'.$this->UNIQ_HASH.'(.*?)'.$this->UNIQ_HASH.'}usx', $text)) {
        ...
        function _ucs2EntitiesDecode($data)
        {
            if (is_array($data)) {
                ...
                if (strpos($data, '%u') !== false) { // improve speed
                    $data = preg_replace_callback('/%u([0-9A-F]{1,4})/usi', array(&$this, '_ucs2EntitiesDecodeCallback'), $data);
                }
