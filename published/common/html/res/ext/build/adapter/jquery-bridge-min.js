...
(function () {
    ...
    Ext.lib.Ajax = function () {
        ...
        return {
            request: function (J, G, E, H, F) {
                var I = {type: J, url: G, data: H, timeout: E.timeout, complete: D(E)};
                if (F) {
                    if (F.xmlData) {
                        I.data = F.xmlData;
                        I.processData = false;
                        ...
                        if (F.jsonData) {
                            I.data = typeof F.jsonData == "object" ? Ext.encode(F.jsonData) : F.jsonData;
                            I.processData = false;
                            I.type = "POST";
                            I.contentType = "text/javascript"
                        }
                    }
                    if (F.headers) {
                        I.beforeSend = function (M) {
                            var K = F.headers;
                            for (var L in K) {
                                if (K.hasOwnProperty(L)) {
                                    M.setRequestHeader(L, K[L])
                                }
                            }
                        }
                    }
                ...
                jQuery.ajax({
                    ...
                    url: H,
                    data: jQuery(I).formSerialize() + (J ? "&" + J : ""),
                    timeout: F.timeout,
                    complete: D(F)
                })
                ...
                return jQuery(E.dom || E).formSerialize()
            }
        }
    }();
