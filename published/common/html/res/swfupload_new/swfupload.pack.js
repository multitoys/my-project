...
function fileQueued(fileObj) {
	try {
		...
		var pendingStr = this.strings.pendingStr ? this.strings.pendingStr : "Pending...";
		progress.SetStatus(pendingStr);
...
function fileDialogComplete(num_files_queued) {
	...
	//this.startUpload();
	if (this.afterFilesSelected != null)
		this.afterFilesSelected ();
	} catch (ex)  {
        this.debug(ex);
	}
}
...
function uploadStart(fileObj) {
	try {
		...
		var progress = new FileProgress(fileObj, this.customSettings.progressTarget);
		var uploadingStr = this.strings.uploadingStr ? this.strings.uploadingStr : "Uploading...";
		progress.SetStatus(uploadingStr);
		progress.ToggleCancel(true, this);
	}
	catch (ex) {}
...
function uploadComplete(fileObj, server_data) {
	try {
		...
		var completeStr = this.strings.completeStr ? this.strings.completeStr : "Complete.";
		progress.SetStatus(completeStr);
		progress.ToggleCancel(false);
		
		if (this.afterFileComplete != null)
			this.afterFileComplete (fileObj);
		
		try {
		/*  I want the next upload to continue automatically so I'll call startUpload here */
		if (this.getStats().files_queued === 0) {
			//document.getElementById(this.customSettings.cancelButtonId).disabled = true;
			if (this.afterCompleted != null)
					this.afterCompleted ();
		} else {
			this.startUpload();
		}
	...
	try {
		...
		switch(error_code) {
			case SWFUpload.UPLOAD_ERROR.HTTP_ERROR:
				if (this.messages != null && this.messages[message])
					progress.SetStatus(this.messages[message]);
				else
			...
			case SWFUpload.UPLOAD_ERROR.FILE_CANCELLED:
				...
				if (this.afterFileCancelled)
					this.afterFileCancelled();
		...
		if (error_code != SWFUpload.UPLOAD_ERROR.FILE_CANCELLED && this.afterFileError != null) {
			this.afterFileError(fileObj, error_code, message);
		}
	} catch (ex) {
        this.debug(ex);
    }
...
function FileProgress(fileObj, target_id) {
	...
	if (!this.fileProgressWrapper) {
		...
		progressText.innerHTML += " <span class='filesize'>(" + getFilesizeStr (fileObj.size) + ")</span>";
...
FileProgress.prototype.SetComplete = function() {
	...
	var oSelf = this;
	setTimeout(function() { oSelf.Disappear(); }, 200);
};
FileProgress.prototype.SetError = function() {
	...
	var oSelf = this;
	setTimeout(function() { oSelf.Disappear(); }, 500);
};
FileProgress.prototype.SetCancelled = function() {
	...
	var oSelf = this;
	oSelf.Disappear()
	//setTimeout(function() { oSelf.Disappear(); }, 100);
};
