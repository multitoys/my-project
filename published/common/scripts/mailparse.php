...
//
    ...
    function parseAddressString($addr, $addName = true)
    {
        if (!trim($addr))
            return false;
        ...
        while ($str != '') {
            ...
            if (!preg_match("/^[0-9a-z-_\.]+@([0-9a-z][0-9a-z-\.]+)\.[a-z]{2,4}$/i", $eml)
                ...
                if ($addName) {
                    ...
                    $prs = preg_replace('/^"\s*(.*?)\s*"$/', "$1", $prs); // deny pair "
                    $prs = preg_replace("/^'\s*(.*?)\s*'$/", "$1", $prs); // deny pair '
...
// Internal subfunction for parseAddressString
    function cut_it($str, &$pr, &$em)
    {
        ...
        if ($p3 !== false && $p3 < $dlm) { // " place before separator => take first part with e-mail
            $p4 = strpos($str, '"', $p3 + 1);
            $p1 = strpos($str, ',', $p4);
            $p2 = strpos($str, ';', $p4);
            if ($p1 === false) $dlm = $p2;
