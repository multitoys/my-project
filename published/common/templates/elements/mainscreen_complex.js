...
WbsEditable = newClass(WbsObservable, {
    ...
    render: function () {
        if (this.mode == WbsEditable.EDIT_MODE) {
            ...
            addHandler(cancelBtn, "click", function (e) {
                this.cancel();
            }, this);
            ...
            valueSpan.onclick = function () {
                if (this.config.clickToEdit) this.setEditMode()
            }.bind(this);
    ...
    render: function () {
        ...
        for (var i = 0; i < pagesCount; i++) {
            ...
            addHandler(pageEl, "click", function (pageEl, pageNo) {
                return function () {
                    pageEl, this.onPageClick(pageEl, pageNo);
                }
            }(pageEl, i), this);
...
WbsTable = newClass(WbsObservable, {
    ...
    constructor: function (config) {
        ...
        if (this.config.autoHeight == false) {
            ...
            this.container = new WbsFlexContainer({
                elem: this.tableElem,
                contentElem: this.renderElem,
                footerElem: this.footerElem
            });
        }
    ...
    setStore: function (store) {
        this.store = store;
        store.addListener("recordModified", function (record) {
            this.storeRecordModified(record)
        }, this);
        store.addListener("dataChanged", function () {
            this.renderData();
            this.fireEvent("afterLoad");
        }, this);
        store.addListener("startLoading", function () {
            this.showLoading();
        }, this);
    ...
    openSortingMenu: function (e, column) {
        ...
        var menu = new SortingMenu({
            items: [
                {
                    label: labels[0], onClick: function () {
                    table.setSortingColumn(column.name, "asc")
                }, iconCls: "sorting-menu-asc"
                },
                {
                    label: labels[1], onClick: function () {
                    table.setSortingColumn(column.name, "desc")
                }, iconCls: "sorting-menu-desc"
                }
            ]
        });
        menu.show(e);
    ...
    createSortLabel: function (column, parentEl, labelValueEl) {
        ...
        var sortingCallback = (column.sortingMenu) ?
            function (e) {
                table.labelClick = true;
                table.openSortingMenu(e, column);
            } :
            function (e) {
                if (table.labelClick) {
                    table.labelClick = false;
                    return;
                }
                ;
                table.setSortingColumn(column.name, (table.currentSorting.direction == "asc") ? "desc" : "asc")
            };
