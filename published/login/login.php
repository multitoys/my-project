<?php
    ...
    if (file_exists("../../kernel/classes/class.accountname.php")) {
        ...
        $dbkey = getDBKey(isset($aliasName) ? $aliasName : getDomainName());
        // ---
    } elseif (file_exists(WBS_DIR."/kernel/wbs.xml") && !file_exists(WBS_DIR.'/published/wbsadmin/html/scripts/multidbkey.php')) {
...
<div style="margin:150px auto; padding: 15px;width:300px;">
    <form name="form" method="post" action="login.php" onSubmit="return switchAction()">
        ...
        <?php if (isset($errorStr) && strlen($errorStr)) {
            ...
            if (strlen($dbkey)) { ?>
                <input name="userdata[DB_KEY]" type="hidden"
                       value="<?php echo htmlspecialchars($userdata["DB_KEY"], ENT_QUOTES); ?>"
                       style="width: 80%;height: 24px;font-size:18px;" maxlength="100">
            <?php } else { ?>
                <span style="white-space: nowrap;">Database Key:</span>
                <br>
                <input name="userdata[DB_KEY]" type="text"
                       value="<?php echo htmlspecialchars($userdata["DB_KEY"], ENT_QUOTES); ?>"
                       style="width: 80%;height: 24px;font-size:18px;" maxlength="100">
                <br>
                <br>
            <?php } ?>
        <span style="white-space: nowrap;"><?php echo $locStrings['app_loginname'] ?><input type="hidden" name="edited"
                                                                                            value="1"></span>
        <br>
        <input name="userdata[U_ID]" type="text" value="<?php echo htmlspecialchars($userdata["U_ID"], ENT_QUOTES); ?>"
               style="width: 80%;height: 24px;font-size:18px;" maxlength="20">
        <br>
        <br>
        ...
        <input name="access" type="checkbox"
               value="ssl" <?php if ($isSSL) echo "checked"; ?>><?= $locStrings['app_use_ssl'] ?>
        <br>
        <a href="forgot.php"
           title="<?= $locStrings['app_forgotpassword_link']; ?>"><?php echo $locStrings['app_forgotpassword_link']; ?></a>
        <br>
        <input name="enter" type="submit" value="<?php echo $locStrings['app_login_btn']; ?>"
               style="font-size:150%;padding:0px auto;margin-left:50%;width:30%;">
        <br>
        <input type="hidden" name="C"
               value="<?php if (isset($_GET['C'])) echo htmlspecialchars($_GET['C'], ENT_QUOTES); ?>">
    </form>
</div>
</body>
</html>