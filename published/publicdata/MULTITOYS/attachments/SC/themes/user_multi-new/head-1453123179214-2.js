...
(function($, undefined){
    ...
    var preview = $(".preview"),
        ...
        content = $("#content"),
        pageNavigator = $("#light-pagination"),
        itemsOnPage = parseInt(pageNavigator.attr("data-itemsOnPage")),
        items = parseInt(pageNavigator.attr("data-items")),
        sort = pageNavigator.attr("data-sort"),
        date = pageNavigator.attr("data-date"),
        made = pageNavigator.attr("data-made"),
        direction = pageNavigator.attr("data-direction"),
    ...
    //};
    //var D = {};
    //D.day = (parseInt(d.day, 10) < 10 ) ? ('0' + d.day) : (d.day);
    //D.month = (parseInt(d.month, 10) < 10 ) ? ('0' + d.month) : (d.month);
    //var z = D.day + '.' + D.month + '.' + D.year;
    //var dateNewPostup = $("#date");
    //var dateNewChina = catPath.find(".china a");
    //var dateNewUkraine = catPath.find(".ukraine a");
    //dateNewPostup.html(z);
    //dateNewPostup.attr("href", "/auxpage_new_items/all/" + date + "/");
    //dateNewChina.attr("href", "/auxpage_new_items/china/" + date + "/");
    //dateNewUkraine.attr("href", "/auxpage_new_items/ukraine/" + date + "/");
    ...
    scrollPane.scroll(function () {
        if (scrollPane.scrollTop() + scrollPane.height() >= content.height() && notProgress) {
            ...
            var itemsOnPage = parseInt(btnMore.attr("data-itemsOnPage"));
            var items = parseInt(btnMore.attr("data-items"));
            var show = parseInt(btnMore.attr("data-show"));
            var page = parseInt(btnMore.attr("data-page"));
            var date = parseInt(btnMore.attr("data-date"));
            var made = btnMore.attr("data-made");
            if (page == -1) {
                page = 0;
            }
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        if (data.result == "success") {
                            ...
                            btnMore.attr("data-show", count_show - page);
    ...
    function realResizeCatalog() {
        var headerHeight = $("#header").height() + $(".header").height() + 5;
        var maincontentHeight = $(window).height() - headerHeight;
        // var columnsHeight = $(document).height() - headerHeight;
        ...
        $("#columns").height(maincontentHeight);
        productLists.height(maincontentHeight);
        try {
            scrollPane.niceScroll({cursorcolor: "coral", cursoropacitymin: 1, cursorwidth: 8});
        } catch (e) {
            console.log( e.name );
    ...
    function resizeCatalog() {
        ...
        setTimeout(realResizeCatalog, 200);
    }
    ...
    function navigation() {
        ...
        var scrollHeight = $(".scroll-pane1").height();
        $(".scroll-pane1").addClass("loader");
        $.ajax({
            url: url + '?ajax=1',
            success: function (data) {
                $('#center').html(data).removeClass("smoke");
                $(".scroll-pane1").removeClass("loader").height(scrollHeight).niceScroll({cursorcolor: "coral", cursoropacitymin: 1, cursorwidth: 8});
                $('a.page-link').click(navigation);
                $(".cart_product_quantity").spin({min: 0, max: 999});
                $(".preview").tooltip({
                    delay: 0, showURL: false, bodyHandler: function () {
    ...
    slideMenu.multiSlideMenu({
        ...
        // minHeightMenu: navMenuHeight,
        afterLoadDone: resizeCatalog
        //$(window).bind('popstate', function () {
        //    $.ajax({
        //        url: location.pathname + '?ajax=1',
        //        success: function (data) {
...
(function ($, undefined) {
   $(function () {
       $("#download").click(function () {
           var query = "/shop/?getFileParam=R2V0WWFuZGV4&download=1";
           if (qt > 0) {
               $.ajax({
                   type: "GET", 
                   url: query, 
