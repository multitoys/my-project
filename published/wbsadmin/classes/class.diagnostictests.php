<?php
    ...
    class DiagnosticTests
    {
        ...
        function my_testConnectivity()
        {
            ...
            $wwwFile = 'http://my.webasyst.net/cc/version.xml';
            $this->setResult("test_connectivity",
                $res = (($fp = fopenTimeout($wwwFile)) && fclose($fp)),
        ...
        function my_testFolderWrite()
        {
            ...
            $writableFoldersPattern = array(
                ...
                'kernel',
                'kernel/includes/smarty/compiled',
                'SC' => 'kernel/includes/smarty/compiled/SC/templates_c/%DB_KEY%',
                'published',
                'published/publicdata',
                'published/publicdata/%DB_KEY%',
                'published/%APP_ID%',
                'published/%APP_ID%/localization',
                'published/wbsadmin',
                            ...
                            if ($res) {
                                ...
                                foreach ($applicationList as $ApplicationId) {
                                    ...
                                    foreach ($tables as $c => $table) {
                                        $c++;
                                        if (mysql_query('SELECT 1 FROM '.$table, $dbh)) {
                                            $succesCount++;
                                        } else {
                                    ...
                                    if (strlen($missedTables)) {
                                        $missedTables = '<br>'.translate('test_tables_missed').':<br>'.$missedTables;
                                    }
        ...
        function getTablesList($applicationId)
        {
            ...
            if (file_exists($file) && ($fileContent = file($file))) {
                ...
                if (preg_match_all('/create[\s]+table[\s]+(if[\s]+not[\s]+exists[\s]+){0,1}[\'`]{0,1}(\w+)[\'`]{0,1}/i', $fileContent, $matches) && isset($matches[2]) && is_array($matches[2])) {
                ...
                if ($files) {
                    foreach ($files as $file) {
                        ...
                        if (file_exists($file) && ($fileContent = file($file))) {
                            $fileContent = implode('', $fileContent);
                            if (preg_match_all('/create[\s]+table[\s]+[\'`]{0,1}(\w+)[\'`]{0,1}/i', $fileContent, $matches)) {
