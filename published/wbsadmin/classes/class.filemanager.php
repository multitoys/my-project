<?php
    ...
    class FileManager
    {
        ...
        function setFile($path = '')
        {
            ...
            if ($this->basePath) {
                $count = 0;
                $this->path = substr(str_replace($this->basePath, '', $this->path, $count), $count ? 1 : 0);
            }
            if (file_exists($path)) {
            ...
            if (file_exists($path) && !is_dir($path) && ($fp = fopen($path, 'rb'))) {
                while ($line = fgets($fp)) {
                    if ($editMode || (($lines++ / $linesPerPage - 1) < $page)) {
                        ...
                        $fileContentStrings[$currentLine] = /*"[{$lines}]:[{$currentLine}] ".*/
                            $line;
                    }
                }
                fclose($fp);
        ...
        function scanDir($dir = '.', $editableFilePattern = null)
        {
            ...
            $count = 0;
            $path = substr(str_replace($this->basePath, '', $dir, $count), $count ? 1 : 0);
            ...
            $pathParts = explode('/', $pathFixed);
            $pathPart = '';
            $directoryInfo['path'][] = array('encoded' => FileManager::encodePath($pathPart), 'name' => str_replace('\\', '/', $this->basePath.'/'));
        ...
        function _sizeToPrint($size, $fileName, $useMb = false)
        {
            ...
            if ($fileName) {
                foreach ($warningSizeMatches as $warningSizeMatche) {
                    if ($size < $warningSizeMatche['size']) {
                        continue;
                    }
