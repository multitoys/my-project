<?php
    ...
    function _convert__PRODUCTS_TABLE($xmlNodeTableArray)
    {
        ...
        while ($row = db_fetch_row($q)) {
            if (trim($row["picture"]) == "" &&
                trim($row["thumbnail"]) == "" &&
                trim($row["big_picture"]) == ""
            )
                continue;
            ss_db_query("insert into ".PRODUCT_PICTURES."( filename, thumbnail, enlarged, productID ) ".
        ...
        ss_db_query("update ".PRODUCTS_TABLE." set categoryID = categoryID + 1 ");
    }
    ...
    function _convert__CATEGORIES_TABLE($xmlNodeTableArray)
    {
        ...
        $q = ss_db_query("select categoryID from ".CATEGORIES_TABLE." order by categoryID DESC ");
        while ($row = db_fetch_row($q)) {
            ss_db_query("update ".CATEGORIES_TABLE." set categoryID = categoryID + 1, ".
                "parent = parent + 1, sort_order = 0 ".
