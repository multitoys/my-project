...
//require(WBS_DIR."includes/Tar.php" );
...
//PRODUCTION VERSION
        /*	const WEB_VERSION_FILENAME	= 'http://my.webasyst.net/cc/version.xml';
	const WEB_CHANGE_FILENAME	= 'http://my.webasyst.net/cc/changelog.xml';
	const WEB_URL				= 'my.webasyst.net';*/
        ...
        function runInstall()
        {
            ...
            switch ($actionType) {
                ...
                case updateManager::ACTION_INSTALL :
                    $res = $this->installScripts();
                    break;
                case updateManager::ACTION_UPDATE :
                    $res = $this->updateSystem(true);
                    break;
            }
...
//		$source=fopenTimeout($url,'r');
            ...
            if ($source && $destiny) {
                ...
                $downloadSize = 4096;
                $sizePacket = max($this->updateSize ? ceil($this->updateSize / 10240000, 0) * 102400 : 102400, 102400);
                $retry_counter = 0;
                do {
                    //sleep(2);
            ...
            if ($readBuf !== false) {
                ...
                if (isset($readBuf['state'])) {//check 30min operation time_out
                    if (in_array($readBuf['state'],
                        ...
                        && ((time() - $readBuf['timeStart']) > 10)
                    ) {//add support for partial extracting
                        switch ($readBuf['state']) {
                            case updateManager::STATE_INSTALL_SCRIPTS: {
                        ...
                        && ((time() - $readBuf['timeStart']) > 50)
                    ) {//add support for partial download //TODO: partial download
                        switch ($readBuf['state']) {
		...
		*/
        ...
        public function updateSystem($force = false, $restart = false)
        {
            ...
            if (PEAR::isError($res)) {
                ...
                $this->log->displayLog();
//		$this->setState(updateManager::STATE_INSTALL_ERROR ,serialize(array('errCode'=>'err_install','msg'=>$res->getMessage())));
	...
	*/
	...
	function check_mysql(&$msg=null )
	{
        ...
        function check_phpversion(&$msg = null)
        {
            ...
            $satisfy = version_compare(PHP_VERSION, '5.0.5', '>=');
        ...
        function check_mysql(&$msg = null)
        {
            ...
            $satisfy = false;
            $ver = mysql_get_client_info();
            $satisfy = version_compare('3.23', PHP_VERSION, '>=');
            ...
            case true : {
                ...
                switch ($actionType) {
                    ...
                    case ACTION_INSTALL :
                        ...
                        break;
                    case ACTION_UPDATE :
                        $res = updateSystem($sysVersion, $updateVersion);
                        break;
                }
