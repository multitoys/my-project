        ...
        //
    {
        ...
        if ($isAdmin) {
            ...
            if ($res) {
                ...
                if (ereg("([0-9]+).([0-9]+).([0-9]+)[-[:alnum:]+]", $ver, $regs)) {

                    if (!(($regs[1] == 3 && $regs[2] >= 23) || $regs[1] >= 4))
                        return PEAR::raiseError($db_strings[59], ERRCODE_APPLICATION_ERR, $_PEAR_default_error_mode, $_PEAR_default_error_options, WBS_ADMIN_USERNAME);
                } else
                    return PEAR::raiseError($db_strings[59], ERRCODE_APPLICATION_ERR, $_PEAR_default_error_mode, $_PEAR_default_error_options, WBS_ADMIN_USERNAME);
            } else
