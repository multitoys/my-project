...
//
    {
        ...
        foreach ($appList as $key => $value) {
            ...
            if (file_exists($destPath) && !$replaceLocFiles) {
                $messageStack[] = sprintf("<font color=blue><b>WARNING:</b></font> ".$db_locStrings[30], $langData[WBS_LANGUAGE_NAME], $app_name);
                continue;
            }

            if (!file_exists($srcPath))
                $messageStack[] = sprintf("<font color=red><b>2ERROR:</b></font> ".$db_locStrings[31], $sys_languages[$src_lang_id][WBS_LANGUAGE_NAME], $app_name);
            ...
            if (strlen($SQL)) {
                ...
                while ($res = mysql_fetch_row($q)) {
                    ...
                    foreach ($res as $value) {
                        $value = mysql_real_escape_string(get_magic_quotes_gpc() ? stripslashes($value) : $value);
                        $str .= (strlen($str) ? ',' : '').($template[$i] ? "'{$value}'" : (($value || ($value === 0) || ($value == '0')) ? floatval($value) : 'NULL'));
                        $i++;
                    }
                    if (strlen($str))
            ...
            while (is_array($insertString = $insertStrings->getItem())) {
                if ($SQLinsert = $insertString['value']) {
                    if (!mysql_unbuffered_query($SQLinsert, $dbh)) {
                        $SQLinsert = htmlentities($SQLinsert);
	...
	*/
        ...
        foreach ($SQLslugStrings as $SQL) {
            ...
            if (!($res = mysql_query($SQLprepare, $dbh))) {
                ...
                while ($row = mysql_fetch_row($res)) {
                    $SQLdata = sprintf($SQLupdate, wbs_make_slug($row[1]), $row[0]);
    ...
    function wbs_copyDirectory($sourcePath, $targetPath, &$errStr = null)
    {
