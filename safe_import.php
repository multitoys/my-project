<?php
    ...
    $setings = array(
            'user'    => SystemSettings::get('DB_USER'),
            'pass'    => SystemSettings::get('DB_PASS'),
            'db'      => SystemSettings::get('DB_NAME'),
            'charset' => 'utf8'
    );
    var_dump($setings);
    $db = new SafeMySQL($setings);
    //var_dump($db);
...
TAG
    ...
    if (($handle = fopen($filename, 'r')) !== false) {
        ...
        define('CAT_NOVINKI_ID', $db->getOne('SELECT categoryID FROM `SC_categories` WHERE name_ru = ?s', 'Новинки'));

        // Идентификатор категории "акция"
        define('CAT_AKCIA_ID', $db->getOne('SELECT categoryID FROM `SC_categories` WHERE name_ru = ?s', 'Акция'));

        // Идентификатор категории "акция "Баллы"
        define('CAT_AKCIA_BALLY_ID', $db->getOne('SELECT categoryID FROM `SC_categories` WHERE name_ru = ?s', 'Акция - Супертрехдневка'));

        // Идентификатор категории "Все бонусные товары"
        define('CAT_BONUS_ID', $db->getOne('SELECT categoryID FROM `SC_categories` WHERE name_ru = ?s', 'Все бонусные товары'));

        // Идентификатор категории "Все товары под заказ"
        define('CAT_ZAKAZ_ID', $db->getOne('SELECT categoryID FROM `SC_categories` WHERE name_ru = ?s', 'Все товары под заказ'));
