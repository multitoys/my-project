...
//----------- Импорт товаров ----------- 
  $filename = $dest_dir.'Conc_search__divolend.csv';
  $file = file($filename);	
  $rowcount = count($file);
...
if (($handle = fopen($filename, "r")) !== FALSE) {
    DeleteRow('Conk_divolend');
    while (($data = fgetcsv($handle, 1000, ";")) !== FALSE) {
	set_time_limit(0);
    $row++;
	$code          = mysql_real_escape_string(DecodeCodepage1251($data[0]));
    $code1c        = mysql_real_escape_string(DecodeCodepage1251($data[1]));

    $productID = GetValue('code_1c', 'Conc_search__divolend', "code = '$code'");
		
    if (!$productID) {
         $query = "
						INSERT INTO Conc_search__divolend
								 (code, code_1c   )
						VALUES   ('$code', '$code1c')"; 
...
// Оптимизация таблиц
  $query = "OPTIMIZE TABLE `Conc_search__divolend`";
  $res   = mysql_query($query) or die(mysql_error()."<br>$query");
  mysql_close(); 
