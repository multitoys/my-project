<?php
    ...
    class CSV
    {
        ...
        public function getInfo()
        {
            ...
            rewind($h);
            $records = array();
            $records[] = $fields = fgetcsv($h, $this->length, $this->delimiter);
            ...
            while ($n <= 10 && $string = fgetcsv($h, $this->length, $this->delimiter)) {
            ...
            if ($fields_count > self::MAX_FIELDS_COUNT) {
                throw new Exception(sprintf(_("Number of columns can not exceed %s"), self::MAX_FIELDS_COUNT));
            }
            ...
            while ($string = fgets($h, $this->length)) {
                // Count only not empty strings
                if (trim(str_replace($this->delimiter, "", $string))) {
                    $n++;
                }
            }
        ...
        public function import($limit = 50)
        {
            if (!$this->handler) {
                ...
                if (!$this->first_line) {
                    $fields = fgetcsv($this->handler, $this->length, $this->delimiter);
                }
            }
            $data = array();
            $i = 0;
            while (!feof($this->handler) && $limit > $i++) {
                $real_data = fgetcsv($this->handler, $this->length, $this->delimiter);
